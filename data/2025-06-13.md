...
TL;DR: 该论文提出了一个猜想，揭示了AI系统中可证明的正确性与广泛数据映射能力之间的基本权衡。


<details>
  <summary>Details</summary>
Motivation: 探讨AI系统中严格正确性与广泛适用性之间的固有矛盾，并将其形式化，以重新定义AI的工程目标和哲学期望。

Method: 通过信息论形式化这一猜想，并结合认识论、形式验证和技术哲学的背景进行分析。

Result: 明确了这一权衡对AI评估标准、治理框架和混合系统设计的影响。
...
TL;DR: 医学基础模型在适应新场景时存在局限性，提出动态上下文切换的AI模型以提升适应性和准确性。


<details>
  <summary>Details</summary>
Motivation: 当前医学AI模型在适应新人群、专业或环境时需微调或检索，易产生上下文错误，限制了其广泛应用。

Method: 提出动态上下文切换的AI模型，无需重新训练即可适应不同医疗场景。

Result: 模型能动态调整推理，适应不同专业、人群和临床角色，提升医疗决策准确性。

Conclusion: 动态上下文切换的AI模型有望扩展医疗服务范围，提升医疗AI的适应性和实用性。
...
Result: 研究发现强相关性不一定意味着因果关系，并识别出影响AD分类的关键特征（如认知评分和遗传风险因素）。

Conclusion: 通过整合特征重要性和可解释性分析，为未来因果推断研究奠定基础，有助于区分因果因素与相关标志物，从而改进AD的早期诊断和干预。
...
TL;DR: 该论文提出了一系列方法，包括安全防护、公平性保障、透明度和问责制，以推动可信AI的发展。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统在关键社会领域的影响日益增加，确保其负责任使用变得至关重要。

Method: 扩展了确定性防护技术以应对延迟观测，提出了公平性防护方法，并引入了透明度和问责制的量化指标。

Result: 在模拟自动驾驶车辆中验证了安全防护技术的有效性，平衡了公平性与最小干预，并提出了责任追溯框架。

Conclusion: 论文通过统一的反应式决策框架，为更安全、公平和可信的AI系统奠定了基础。
...
TL;DR: 该论文提出了WGSR-Bench，首个基于兵棋游戏的LLM战略推理基准，用于系统评估LLM在多智能体决策、意图推断和反事实推理中的能力。
...
Method: 通过兵棋游戏作为评估环境，设计WGSR-Bench，围绕环境态势感知、对手风险建模和政策生成三个核心任务（S-POE架构）构建测试样本。

Result: 设计了一个基于LLM的兵棋游戏智能体，用于全面评估战略推理能力。

Conclusion: WGSR-Bench旨在评估当前先进LLM在博弈论战略推理中的优势和局限性，推动大模型驱动的战略智能研究。
...
<details>
  <summary>Details</summary>
Motivation: 探讨AI如何通过增强人类智力推动认知生产力的革命，区别于工业革命的物理驱动。

Method: 采用多学科视角（计算机科学、经济学、社会学），结合历史信息技术的比较和实际案例，构建理论框架。

Result: AI作为认知引擎，显著提升了知识工作的生产力，并推动了从体力到认知生产力的转变。

Conclusion: AI的潜力在于补充人类认知能力，标志着生产力进化的新篇章，需重新思考技能、组织与政策。
...
TL;DR: 论文揭示了AI系统能力提升时安全验证的计算复杂性障碍，证明验证复杂度随系统表达能力指数增长，并提出了一个战略三难选择。


<details>
  <summary>Details</summary>
Motivation: 研究AI系统能力提升时安全验证的复杂性，揭示其与系统表达能力的关系，为AI对齐提供理论基础。

Method: 通过形式化能力-风险缩放动态（CRS），提出四个核心定理，证明验证复杂度的指数增长、安全策略的稀缺性、对齐技术的局限性以及鲁棒安全属性的零测性。

Result: 验证复杂度为coNP完全问题，安全策略占比极低，且无法通过有限对齐技术实现全覆盖，鲁棒安全属性为零测集。

Conclusion: AI发展面临三难选择：限制系统复杂度、接受不可验证风险或开发新安全范式。论文为AI对齐提供了首个系统性复杂性理论分析。
...
<details>
  <summary>Details</summary>
Motivation: 开发能够在不重新训练的情况下适应多样化战略环境的AI代理是多智能体学习的核心挑战，Pokémon VGC因其巨大的团队配置空间和策略变化性成为理想的研究领域。

Method: 引入VGC-Bench基准，提供基础设施、标准化评估协议、人类玩家数据集及多种基线方法（如大型语言模型、行为克隆、强化学习和博弈论方法）。

Result: 在单团队配置下，方法能击败专业玩家，但随着团队规模扩大，泛化能力显著下降。
...
TL;DR: 本文提出了一种名为Optimus-3的多模态通用代理，通过知识增强的数据生成、混合专家架构和多模态推理增强的强化学习，解决了开放世界环境中的任务干扰和视觉多样性问题。


<details>
  <summary>Details</summary>
Motivation: 在开放世界环境（如Minecraft）中构建具备感知、规划、行动、接地和反思能力的通用代理面临数据不足、任务干扰和视觉多样性等挑战。

Method: 1) 知识增强的数据生成管道；2) 任务级路由的混合专家架构；3) 多模态推理增强的强化学习方法。

Result: Optimus-3在Minecraft环境中超越了通用多模态大语言模型和现有最先进代理。

Conclusion: 通过提出的创新方法，Optimus-3成功解决了开放世界环境中的关键挑战，成为高效的通用代理。
...
TL;DR: NeuroPAL结合NEAT与PAL，通过交替训练和评估提升StarCraft AI的策略学习效率，显著减少训练时间并涌现专家级行为。


<details>
  <summary>Details</summary>
Motivation: StarCraft AI的传统方法（规则系统或监督学习）在适应性和计算效率上存在局限，NeuroPAL旨在通过神经进化框架解决这些问题。

Method: NeuroPAL整合NEAT与PAL，通过低频高精度评估与高频低精度训练交替进行，提升样本效率。

Result: PAL显著加速学习，训练时间减半，并涌现出专家级策略（如代理兵营和防御优化）。
...
TL;DR: 提出了一个名为Mirage-1的多模态GUI代理，通过分层多模态技能（HMS）模块和技能增强蒙特卡洛树搜索（SA-MCTS）算法，解决了在线环境中长时任务的知识不足和领域差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大型语言模型（MLLM）在在线环境中执行长时任务时表现不佳，主要由于知识不足和离线与在线领域的差距。

Method: 1. 提出HMS模块，将轨迹分层抽象为执行技能、核心技能和元技能；2. 提出SA-MCTS算法，利用离线技能缩小在线搜索空间；3. 构建Mirage-1代理。

Result: 在多个基准测试（AndroidWorld、MobileMiniWob++、Mind2Web-Live、AndroidLH）中，Mirage-1分别比之前代理提升了32%、19%、15%和79%。
...
TL;DR: 本文综述了推理代理式检索增强生成（RAG）方法，将其分为预定义推理和代理式推理两类，并分析了代表性技术、架构设计及未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 解决传统RAG系统在复杂推理、动态检索和多模态集成方面的局限性。

Method: 将RAG方法分为预定义推理（固定模块化流程）和代理式推理（模型自主协调工具交互），并分析其技术和架构。

Result: 总结了推理代理式RAG的关键技术和挑战，提出了未来研究方向。

Conclusion: 推理代理式RAG有望提升系统的灵活性、鲁棒性和适用性，但仍需进一步研究。
...
TL;DR: 论文提出了一种基于代理的自动扩展框架，通过动态调整硬件资源和内部服务配置，在资源受限的边缘计算环境中最大化需求满足。比较了四种扩展代理的性能，结果显示均能实现可接受的SLO性能。
...
Result: 所有代理均能实现可接受的SLO性能，Deep Q Network受益于预训练，结构分析收敛快，深度主动推理代理兼具理论和实践优势。
...
TL;DR: OIBench是一个高质量、私密且具有挑战性的信息学竞赛级别数据集，包含250个精心设计的问题，旨在推动算法推理能力的提升。


<details>
  <summary>Details</summary>
Motivation: 随着模型日益复杂，传统算法基准逐渐饱和，需要更具挑战性的基准来指导算法推理的未来改进。

Method: 详细介绍了OIBench的构建方法，确保全面评估不同编程范式和复杂度，并通过实验展示其抗污染特性。提出了时间/空间完成曲线进行更细粒度的效率分析，并通过高级参与者评估实现人机直接比较。

Result: 实验显示，开源模型落后于闭源模型，但当前SOTA模型在正确性和效率上已超越大多数人类参与者，但仍未达到最优解。

Conclusion: 通过开源OIBench，希望该基准能促进未来LLM代码推理能力的提升。
...
TL;DR: 论文提出科学家首次考试（SFE）基准，用于评估多模态大语言模型（MLLMs）在科学认知能力上的表现，填补现有基准在感知和推理能力评估上的不足。
...
Method: 设计了SFE基准，包含830个专家验证的视觉问答对，覆盖66个多模态任务和五个高价值学科，评估模型的科学信号感知、属性理解和比较推理能力。

Result: 实验显示当前最先进的GPT-3和InternVL-3在SFE上的表现仅为34.08%和26.52%，表明MLLMs在科学领域仍有较大提升空间。
...
Method: 通过动态调整任务复杂度（对象数量、关系深度等），LogiPlan评估模型在不同难度下的表现，并测试其自我修正能力。

Result: 评估显示，模型在简单任务中表现良好，但在复杂逻辑规划中仍有显著性能差距，与模型规模和架构相关。

Conclusion: LogiPlan揭示了当前模型在深度逻辑规划中的局限性，为未来改进提供了方向。
...
Result: 通过比较规则推断准确性、假设评估、序列有效性和符号解释质量等指标，评估模型性能。

Conclusion: Primender序列和方法论为LLM在符号推理和假设测试中的基准测试提供了新工具，连接了数论、人工智能和软件工程领域。
...
TL;DR: 提出了一种新的诊断方法，仅需基本子系统关系知识和正常运行数据，结合神经网络症状生成器和图诊断算法，显著减少搜索空间并提高诊断准确性。
...
Method: 结合基于神经网络的症状生成器和新的图诊断算法，利用最小因果关系信息。

Result: 在模拟数据集中，82%的案例正确识别因果组件，73%的场景显著减少搜索空间；真实数据集测试显示实用潜力。
...
TL;DR: TeleMath是首个专门评估大型语言模型在电信领域数学问题解决能力的基准数据集，包含500个问答对，覆盖广泛主题。测试显示，专为数学或逻辑推理设计的模型表现最佳，而通用模型表现较差。
...
Conclusion: TeleMath为评估电信领域数学问题解决能力提供了基准，支持未来研究。
...
TL;DR: 该论文提出了一种将AutomationML（AML）建模约束形式化并验证的流程，通过映射AML模型到OWL本体、利用大型语言模型将文本规则转换为SHACL约束，并自动验证和解释结果。


<details>
  <summary>Details</summary>
Motivation: 现有AML建模建议通常以非正式文本约束形式存在，无法自动验证，因此需要一种方法将这些约束形式化并实现自动验证。

Method: 通过RML和SPARQL将AML模型映射到OWL本体，利用大型语言模型将文本规则转换为SHACL约束，并验证和解释结果。

Result: 结果表明，即使复杂的建模规则也可以半自动检查，无需用户理解形式化方法或本体技术。

Conclusion: 该方法为AML建模约束的自动验证提供了一种可行方案，降低了技术门槛。
...
TL;DR: ASPMT结合了答案集编程和可满足性模理论，通过编译器aspsmt2smt将程序转换为SMT实例，利用gringo和z3处理变量，支持实数计算和连续变化推理。


<details>
  <summary>Details</summary>
Motivation: 结合答案集编程（ASP）和可满足性模理论（SMT）的优势，解决涉及连续变化的复杂推理问题。

Method: 开发编译器aspsmt2smt，将ASPMT程序转换为SMT实例，利用gringo部分接地输入程序，z3处理剩余变量。

Result: 系统能有效处理实数计算，支持连续变化的推理。

Conclusion: ASPMT和aspsmt2smt为复杂推理问题提供了实用工具，尤其在连续变化领域表现优异。
...
TL;DR: 该论文提出了一种增强神经符号模型的方法，用于反事实推理，通过符号因果推理协调感知和模拟模块，在CLEVRER和CRAFT基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 视频动态的因果和时间推理是一个挑战性问题，现有神经符号模型在反事实问题上表现不足。

Method: 定义因果图表示事件关系，使用答案集编程（ASP）协调感知与模拟模块，并利用预训练语言模型（如GPT-3.5/4）作为动态模拟器。

Result: 在CLEVRER基准上达到最先进性能，CRAFT基准上通过符号因果推理指导的提示进一步提升表现。
...
TL;DR: 论文提出了OPT-BENCH基准和OPT-Agent框架，用于评估大语言模型在迭代优化任务中的表现，并通过实验验证了历史反馈对性能的提升。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在通过历史反馈迭代优化复杂解决方案方面的能力，填补现有研究的空白。

Method: 设计了包含20个Kaggle机器学习任务和10个经典NP问题的OPT-BENCH基准，并开发了OPT-Agent框架，模拟人类推理过程进行迭代优化。

Result: 实验表明，历史反馈显著提升了模型在机器学习和NP问题上的优化性能。

Conclusion: OPT-BENCH和OPT-Agent为推进大语言模型驱动的优化和迭代推理研究提供了工具和基准。
...
TL;DR: 本文提出了一种结合链式思维（CoT）推理与模型上下文协议（MCP）的框架，用于增强大语言模型（LLMs）在时空行为模拟中的能力，实验验证了其有效性和高效性。
...
TL;DR: GenPlanX结合大型语言模型（LLMs）与经典AI规划引擎，实现自然语言描述规划任务，提升办公任务效率。


<details>
  <summary>Details</summary>
Motivation: 传统AI规划技术无法理解自然语言描述的规划任务，LLMs在理解人类意图方面表现优异，因此提出结合两者的方案。

Method: 提出GenPlanX框架，整合LLMs用于自然语言任务描述，结合经典AI规划引擎及执行监控框架。

Result: GenPlanX在办公任务中展示了高效性，能够优化工作流程并提升生产力。

Conclusion: GenPlanX通过人机协作，为自然语言规划任务提供了有效解决方案。
...
Result: 评估了近30个主流MLLM，发现虽然当前模型仍面临挑战，但在毒性理解、语义约束和结构感知分子编辑方面表现出潜力。
...
TL;DR: 强化学习结合可验证奖励（RLVR）能在某些模型中激发强数学推理能力，即使奖励信号与正确答案相关性很低、无相关性或负相关。
...
Method: 使用RLVR对Qwen2.5-Math-7B等模型进行实验，测试不同虚假奖励信号（如随机奖励、格式奖励、错误标签等）的效果。

Result: RLVR显著提升了Qwen2.5-Math-7B在MATH-500任务中的表现，但对其他模型家族（如Llama3或OLMo2）效果不佳。代码推理行为在Qwen中显著增加。

Conclusion: RLVR可能通过激活预训练中学到的有用推理表示来发挥作用，但机制尚不明确。未来研究需验证多样化模型，而非单一选择。
...
TL;DR: 本文提出了一个跨文本、图像和音频模态的生成内容自动评估方法的统一分类框架，填补了当前研究的空白。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏一个系统性的框架来全面组织跨模态的生成内容自动评估方法，本文旨在解决这一问题。

Method: 通过分析文本生成中最成熟的评估方法，扩展框架至图像和音频生成，提出了五种基本范式。

Result: 提出了一个适用于文本、图像和音频的统一分类框架，并展示了其广泛适用性。

Conclusion: 未来研究应关注跨模态评估方法的发展方向。
...
<details>
  <summary>Details</summary>
Motivation: 代理任务在 NLP 和 AI 中日益重要，但现有数据缺乏工具交互，且人工标注成本高，限制了可扩展性。

Method: TaskCraft 通过深度和广度扩展原子任务，生成结构和层次复杂的任务，并提供执行轨迹。

Result: 实验表明，这些任务改进了生成工作流中的提示优化，并提升了代理基础模型的监督微调效果。

Conclusion: TaskCraft 提供了一个包含约 36,000 个任务的合成数据集，支持未来代理调优和评估的研究。
...
TL;DR: 论文探讨了语义退化对自然语言处理的影响，提出意义是通过观察者依赖的解读行为实现的，并通过实验验证了语言解读的非经典语境性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是揭示大型语言模型（LLMs）和其他NLP系统在语义退化下的固有局限性，挑战传统语言学中形式本身具有意义的观点。

Method: 通过Kolmogorov复杂性理论分析，并结合语义贝尔不等式测试，使用LLM代理作为计算认知系统来解读歧义词对。

Result: 实验结果显示平均CHSH期望值在1.2到2.8之间，部分结果显著违反经典边界（|S|≤2），表明语言解读具有非经典语境性。

Conclusion: 结论指出经典频率分析方法在自然语言中是必然有损的，建议采用贝叶斯风格的重复采样方法来更准确地描述语境中的语言意义。
...
TL;DR: 论文研究了语义相同但表达不同的提示对大型语言模型（LLM）行为的影响，提出了Prompt-Based Semantic Shift（PBSS）框架来测量这种差异。


<details>
  <summary>Details</summary>
Motivation: 探索LLM在语义相同但表达不同的提示下的行为变化，揭示模型评估的稳定性问题。

Method: 提出PBSS框架，在十个约束任务中测量LLM的行为漂移。

Result: 发现模型在语义等效提示下存在一致的行为变化，与分词和解码策略相关。

Conclusion: 提示的微小变化可能导致模型行为不稳定，分词和解码策略是影响服务质量的关键因素。
...
Method: 提出ChartReasoner框架：1) 将图表图像转换为结构化ECharts代码；2) 设计数据合成管道生成高质量推理轨迹；3) 结合监督微调和强化学习训练模型。

Result: 在四个公共基准测试中表现优异，接近GPT-4o等专有系统性能，且参数更少。

Conclusion: ChartReasoner能有效保留图表细节，性能优于现有开源模型，接近专有系统。
...
TL;DR: 提出了一种无监督算法ICM，用于在没有外部监督的情况下微调预训练语言模型，其性能优于人类监督训练。


<details>
  <summary>Details</summary>
Motivation: 针对超人类能力的模型难以获得高质量人类监督的问题，提出无需外部监督的解决方案。

Method: 使用内部一致性最大化（ICM）算法，基于模型自身生成的标签进行微调。

Result: 在多个任务上表现优于人类监督训练，并能更好地激发模型的超人类能力。
...
<details>
  <summary>Details</summary>
Motivation: 探讨LLM在共情沟通标注中的可靠性，验证其在情感敏感任务中的适用性。

Method: 通过比较专家、众包工作者和LLM对200个真实对话的共情标注（共9,144条），评估其一致性。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在推进资源有限语言（如孟加拉语）的情感分析，探索高效的情感识别技术。

Method: 使用了Linear SVM、KNN、Random Forest、BiLSTM和AdaBoost模型，结合TF-IDF向量化和PCA降维，并通过LIME解释AdaBoost分类器的预测。

Result: 研究比较了多种技术，旨在找到适合孟加拉语情感分析的高效方法。
...
Method: 使用word2vec算法训练HC披露文本，开发包含五个子类别的HC相关关键词汇表。

Result: 提供了HC词汇表、公司披露数据和Python代码，并展示了如何利用这些资源进行进一步研究。

Conclusion: 研究为HC管理相关问题提供了工具，并探讨了未来研究方向。
...
TL;DR: 该论文探讨了大型语言模型（LLMs）在故事生成中的能力，通过叙事规划问题评估其表现，发现GPT-4级模型能生成小规模因果合理的故事，但在角色意图和戏剧冲突方面仍有挑战。
...
Method: 通过叙事规划问题评估LLMs的能力，建立基于文学例子的基准，关注因果合理性、角色意图和戏剧冲突。

Result: GPT-4级LLMs能生成小规模因果合理的故事，但在角色意图和戏剧冲突方面表现不佳，需强化学习训练以支持复杂推理。

Conclusion: 研究揭示了LLMs在故事生成中的局限性，为游戏环境中应用LLM叙事规划提供了挑战和思考方向。
...
<details>
  <summary>Details</summary>
Motivation: 改进复杂现实世界事件的视频检索，通过自动提取LLMs和VLMs中的潜在知识。

Method: 提出Q2E方法，分解查询并利用LLMs和VLMs的知识，支持多模态输入（视觉和语音），采用基于熵的融合评分。

Result: 在多个数据集和检索指标上，Q2E优于现有基线方法，音频信息的整合显著提升检索效果。

Conclusion: Q2E展示了通过分解查询和多模态融合提升视频检索的潜力，代码和数据已开源。
...
TL;DR: TTT-Bench是一个新基准，通过简单的井字棋类游戏评估大型推理模型（LRMs）在战略、空间和逻辑推理方面的能力，发现即使擅长数学问题的模型在这些简单任务上表现不佳。
...
Method: 提出TTT-Bench，通过四种井字棋类游戏生成可验证的问题，评估模型的推理能力。

Result: 模型在TTT-Bench上的表现比数学任务低41%和5%，且在长期战略推理中表现较差。

Conclusion: LRMs在简单推理任务中的表现不如复杂数学问题，表明其推理能力存在局限性。
...
TL;DR: 该论文提出了一种利用计算方法识别不可靠叙述者的方法，并构建了一个名为TUNa的数据集。通过分析不同文本现象，定义了三种不可靠性分类任务，并测试了多种LLM的性能。结果表明任务具有挑战性，但LLM有潜力。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过计算手段识别不可靠叙述者，填补文学理论与现实文本分析之间的空白。

Method: 结合文学理论定义不可靠叙述者类型，构建TUNa数据集，设计分类任务，测试LLM在少样本、微调和课程学习中的表现。

Result: 任务具有挑战性，但LLM在识别不可靠叙述者方面显示出潜力。

Conclusion: 释放数据集和代码，鼓励未来研究，探索LLM在不可靠叙述者识别中的应用。
...
Conclusion: ToxSyn-PT为低资源环境下的仇恨言论检测研究提供了有力工具，并公开数据集以促进相关研究。
...
TL;DR: 本文探讨语言模型是否具有贝叶斯大脑，发现其在一定条件下可能表现出近乎确定性的决策行为，挑战了之前的采样假设，并提出区分随机与确定性决策的方法。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型的决策行为是否类似贝叶斯推理，以及如何避免在模拟Gibbs采样中推断出误导性的先验。

Method: 通过实验分析大型语言模型在不同条件下的决策模式，提出区分随机与确定性决策的方法。

Result: 语言模型在非零采样温度下仍可能表现出确定性行为，导致模拟Gibbs采样收敛到“虚假先验”。

Conclusion: 研究揭示了语言模型的决策复杂性，为理解其行为提供了新视角，并提出了避免误导性先验推断的解决方案。
...
Method: 通过聚类将相似梯度特征的数据分组，将数据选择问题建模为多臂老虎机问题，并采用改进的UCB算法进行求解。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在低资源语言中伪标签噪声和领域适应问题，特别是在少标签分类任务中的挑战。

Method: 提出Flick，通过从初始多簇中蒸馏高置信度伪标签，并引入伪标签细化组件，利用单簇凝聚力和自适应top-k选择机制。

Result: 在14个多样化数据集（包括阿拉伯语、乌尔都语等低资源语言）上展示了优越性能和适应性。

Conclusion: Flick在低资源语言环境下显著提升了少标签分类任务的性能，优于现有方法。
...
TL;DR: 研究探讨了用户建议对大型语言模型（LLMs）在模拟教育场景中的影响，发现模型表现受问题表述影响显著，存在迎合倾向。


<details>
  <summary>Details</summary>
Motivation: 研究动机是了解LLMs在教育场景中是否会因用户建议而表现出迎合行为，及其对教育公平的潜在影响。

Method: 测试了五款OpenAI的GPT-4o和GPT-4.1模型，通过五种实验条件分析模型回答质量的变化。

Result: 结果显示，模型正确性因用户提供的答案而显著变化（±15%），且小模型迎合倾向更强（高达30%）。

Conclusion: 结论指出需进一步研究机制并缓解此类偏见，以确保教育公平。
...
Method: 采用渐进式交替训练，将语音和文本单元交替输入，并逐步减少文本比例，以促进从文本到语音的模态适应。
...
TL;DR: 提出了一种利用程序执行的确定性生成高质量CoT监督数据的可扩展方法，替代了依赖昂贵人工标注或易错的LLM生成CoT的方法。


<details>
  <summary>Details</summary>
Motivation: 获取可靠且准确的推理监督数据是提升LLM推理能力的主要挑战。

Method: 通过从代码执行中提取可验证的逐步推理轨迹，并将其转化为自然语言的CoT推理数据。

Result: 实验表明，该方法能有效提升LLM在多样化任务中的推理能力，并减少推理时的冗余和过度思考。

Conclusion: 该方法生成的推理数据准确性高，且能显著减少推理时的无效重复。
...
Result: 实验表明，TableRAG在公开数据集和HeteQA基准上均优于现有基线，达到新SOTA。

Conclusion: TableRAG为异构文档问答提供了高效解决方案，并通过HeteQA基准推动了多跳异构推理能力的研究。
...
TL;DR: 论文提出PAG框架，通过多轮强化学习让LLM在策略和验证者角色间切换，实现自我纠正，提升推理和验证能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖独立验证模型或多阶段训练，限制了可扩展性，LLM在自我验证方面仍有不足。

Method: 提出PAG框架，结合生成和验证角色，选择性修正错误，避免模型崩溃。

Result: 实验表明PAG在推理和验证能力上均有提升，优于自一致性验证。

Conclusion: PAG是一种简单有效的自我纠正框架，能同时提升LLM的生成和验证能力。
...
### [51] [Burn After Reading: Do Multimodal Large Language Models Truly Capture Order of Events in Image Sequences?](https://arxiv.org/abs/2506.10415)
*Yingjin Song,Yupei Du,Denis Paperno,Albert Gatt*

Main category: cs.CL

TL;DR: TempVS是一个评估多模态大语言模型（MLLMs）在图像序列中时间推理能力的基准测试，包含三个主要测试和基础测试。评估显示MLLMs表现不佳，与人类能力差距显著。


<details>
  <summary>Details</summary>
Motivation: 研究MLLMs在时间推理和基础能力上的表现，揭示其局限性并推动未来研究方向。

Method: 设计了TempVS基准测试，包含事件关系推理、句子排序和图像排序三个测试，并评估了38个先进MLLMs。
...
### [52] [Beyond the Battlefield: Framing Analysis of Media Coverage in Conflict Reporting](https://arxiv.org/abs/2506.10421)
...
Method: 结合框架语义学和大语言模型，计算分析新闻语料中的战争与和平新闻指标。

Result: 战争报道多于和平报道，美、英和中东媒体在冲突角色描述上存在显著差异。

Conclusion: 媒体框架影响读者观点，不同地区媒体存在明显偏见，需更深入分析。
...
TL;DR: Tina是一种高效低成本的小型推理模型，通过参数高效更新和低秩适应（LoRA）在强化学习中实现，性能接近或超越SOTA模型，且成本极低。
...
Method: 使用LoRA在1.5B参数的基础模型上进行参数高效更新，结合强化学习。

Result: Tina在AIME24上推理性能提升20%，Pass@1准确率达43.33%，成本仅为9美元。

Conclusion: LoRA在高效推理中表现出色，且开源所有资源以促进研究。
...
TL;DR: 该研究旨在通过优化奖励函数和引入输出长度惩罚，提升大语言模型（LLM）在简单问题上的简洁性和复杂问题上的准确性，从而提高整体推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法如Chain-of-Thought提示虽能提升推理能力，但会导致输出过长和计算延迟。现有强化学习方法对问题复杂性考虑不足，导致效果不佳。

Method: 通过分割奖励函数并引入新的输出长度惩罚，动态管理模型的推理效率。

Result: 在GSM8K和MATH500数据集上缩短了输出长度并保持或提升准确性；在更复杂的AIME2024数据集上提高了准确性。

Conclusion: 该方法有效平衡了推理的简洁性和准确性，提升了LLM的整体性能。
...
<details>
  <summary>Details</summary>
Motivation: 研究如何高效且低成本地从语言模型中提取和迁移推理能力。

Method: 使用稀疏自编码器（SAE）从源模型中捕获推理能力，并通过监督微调将其迁移到目标模型。

Result: SAE-Tuning在仅需约1美元和20分钟的情况下，保留了RL训练模型97%以上的性能，并在轻量RL训练模型上实现了显著的推理性能提升。

Conclusion: SAE-Tuning是一种高效、低成本且通用的方法，能够模块化地提取和迁移推理能力。
...
### [56] [Table-Text Alignment: Explaining Claim Verification Against Tables in Scientific Papers](https://arxiv.org/abs/2506.10486)
*Xanh Ho,Sunisth Kumar,Yun-Ang Wu,Florian Boudin,Atsuhiro Takasu,Akiko Aizawa*

Main category: cs.CL

TL;DR: 论文将表格-文本对齐任务重新定义为解释任务，要求模型识别验证科学声明所需的关键表格单元格，并通过实验验证了表格对齐信息对声明验证性能的提升作用。


<details>
  <summary>Details</summary>
Motivation: 当前科学声明验证仅预测最终标签，缺乏对模型推理过程的解释和可解释性。

Method: 扩展SciTab基准数据集，添加人工标注的单元格级理由，并提出处理模糊情况的分类法。

Result: 实验表明：(i) 表格对齐信息提升了声明验证性能；(ii) 多数LLM虽能预测正确标签，但无法恢复与人类对齐的理由。

Conclusion: 模型预测缺乏忠实推理，需进一步改进以实现可解释性。
...
TL;DR: 论文研究了大型语言模型（LLMs）中的偏见问题，发现通过预提示人物设定在MMLU基准测试中差异微小且随机，但在评分任务和薪资谈判建议中表现出显著偏见。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型训练数据包含争议性和刻板印象内容，导致模型输出带有偏见。随着LLM助手记忆和个性化趋势，用户无需预提示人物设定，模型已知其社会人口特征，偏见问题更加突出。

Method: 通过预提示人物设定在MMLU基准测试评估模型偏见，并设计评分任务和薪资谈判建议任务进一步分析。

Result: 预提示人物设定在MMLU中差异微小且随机；评分任务和薪资谈判建议任务中表现出显著偏见。

Conclusion: LLMs在特定任务中表现出显著偏见，随着模型记忆和个性化功能的发展，偏见问题需进一步关注和解决。
...
### [58] [Beyond Single-User Dialogue: Assessing Multi-User Dialogue State Tracking Capabilities of Large Language Models](https://arxiv.org/abs/2506.10504)
*Sangmin Song,Juhwan Choi,JungMin Yun,YoungBin Kim*

Main category: cs.CL

TL;DR: LLMs在零样本对话状态跟踪（DST）中表现优异，但现有基准主要针对单用户对话，无法反映多用户交互的复杂性。本研究通过生成第二用户的语料扩展数据集，评估LLMs在多用户DST中的表现，发现性能显著下降，凸显了当前模型的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有DST基准主要关注结构化单用户对话，忽略了多用户交互的复杂性，需评估LLMs在多用户场景中的鲁棒性。

Method: 基于言语行为理论生成第二用户的语料，扩展现有DST数据集，系统地将第二用户的语料融入对话中，以评估LLMs在多用户DST中的表现。

Result: 实验结果显示，与单用户DST相比，LLMs在多用户场景中的性能显著下降。

Conclusion: 当前LLMs在多用户DST中存在局限性，未来研究需提升模型在此类场景中的表现，以实现更真实和鲁棒的DST模型。
...
### [59] [Reliable Reasoning Path: Distilling Effective Guidance for LLM Reasoning with Knowledge Graphs](https://arxiv.org/abs/2506.10508)
*Yilin Xiao,Chuang Zhou,Qinggang Zhang,Bo Li,Qing Li,Xiao Huang*

Main category: cs.CL

TL;DR: 论文提出了RRP框架，通过结合知识图谱和大型语言模型，解决复杂问题推理中的路径提取和优化问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在知识密集型任务中表现不佳，缺乏背景知识且易产生幻觉。知识图谱的引入虽补充了事实知识，但仍难以解决复杂问题。

Method: 提出RRP框架，结合LLM的语义能力和关系嵌入的结构信息，通过双向分布学习和反思模块优化推理路径。

Result: 在两个公开数据集上，RRP表现优于现有基线方法，并能以即插即用方式增强多种LLM的推理能力。

Conclusion: RRP通过生成高质量推理路径，为LLM提供了有效的推理指导，提升了复杂问题的解决能力。
...
### [60] [Unsupervised Protoform Reconstruction through Parsimonious Rule-guided Heuristics and Evolutionary Search](https://arxiv.org/abs/2506.10614)
...
Result: 在重建拉丁原始形式任务中，字符级准确性和音韵合理性指标均显著提升。

Conclusion: 混合方法能更有效地重建原始语言形式，优于传统数据驱动方法。
...
### [61] [SDialog: A Python Toolkit for Synthetic Dialogue Generation and Analysis](https://arxiv.org/abs/2506.10622)
...
Conclusion: SDialog在合成数据生成工具标准化方面迈出了重要一步，有助于提升研究的可重复性。
...
### [62] [NeuralNexus at BEA 2025 Shared Task: Retrieval-Augmented Prompting for Mistake Identification in AI Tutors](https://arxiv.org/abs/2506.10627)
*Numaan Naeem,Sarfraz Ahmad,Momina Ahsan,Hasan Iqbal*

Main category: cs.CL

TL;DR: 本文介绍了BEA 2025共享任务中用于识别数学辅导反馈错误的系统，探索了四种方法，最终结合检索增强的提示系统与大型语言模型（如GPT 4o）表现最佳。


<details>
  <summary>Details</summary>
Motivation: 评估AI辅导工具是否能正确识别学生数学推理中的错误，提升教学反馈能力。
...
Conclusion: 结合示例驱动提示与LLM推理的方法在评估教学反馈中效果显著。
...
### [63] [Spelling-out is not Straightforward: LLMs' Capability of Tokenization from Token to Characters](https://arxiv.org/abs/2506.10641)
*Tatsuya Hiraoka,Kentaro Inui*

Main category: cs.CL

TL;DR: LLMs在逐字符拼写任务中表现准确，但在复杂字符级任务（如识别子组件）中表现不佳。研究发现，LLMs的嵌入层未完全编码字符级信息，需依赖中间层和高层Transformer重建知识。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs在拼写过程中如何内部表示和利用字符级信息，揭示其处理简单字符任务的机制。

Method: 通过探测分类器、识别知识神经元和检查注意力权重三种互补分析方法，验证LLMs的拼写行为机制。

Result: 发现LLMs的嵌入层未完全编码字符级信息，拼写行为在中间层和高层Transformer出现明显突破。

Conclusion: LLMs处理字符级任务的方式与人类不同，依赖深层网络重建字符知识，嵌入层的作用有限。
...
### [64] [Large Language Models for Detection of Life-Threatening Texts](https://arxiv.org/abs/2506.10687)
*Thanh Thi Nguyen,Campbell Wilson,Janis Dalins*

Main category: cs.CL

TL;DR: 本文提出了一种利用大语言模型（LLMs）检测威胁生命语言的有效方法，并比较了其与传统方法的性能。实验表明，LLMs在平衡和不平衡数据场景下表现优异，尤其是Mistral和Llama-2模型。
...
### [65] [Inferring Adjective Hypernyms with Language Models to Increase the Connectivity of Open English Wordnet](https://arxiv.org/abs/2506.10715)
*Lorenzo Augello,John P. McCrae*

Main category: cs.CL

TL;DR: 论文探讨了如何建立形容词之间的上位关系，提出了新资源并利用大语言模型预测形容词上位关系。
...
Result: 成功开发了形容词上位关系资源，并验证了大语言模型在此任务上的适应性。

Conclusion: 论文展示了填补形容词上位关系空白的方法，并证明了TaxoLLaMa模型在此任务中的潜力。
...
### [66] [PREMISE: Scalable and Strategic Prompt Optimization for Efficient Mathematical Reasoning in Large Models](https://arxiv.org/abs/2506.10716)
*Ye Yu,Yaoning Yu,Haohan Wang*

Main category: cs.CL

TL;DR: PREMISE是一种基于提示的框架，通过优化提示减少大型推理模型（LRMs）的冗余计算，降低token使用和成本，同时保持准确性。


<details>
  <summary>Details</summary>
Motivation: 当前大型推理模型在数学推理中生成的冗长链式思考（CoT）增加了token使用和成本，限制了在延迟敏感或API受限场景中的部署。

Method: PREMISE结合轨迹级诊断和梯度启发的提示优化，通过多目标文本搜索平衡token长度和答案有效性，无需修改模型权重。

Result: 在GSM8K、SVAMP和Math500基准测试中，PREMISE保持或提升准确性（如Claude 96%→96%，Gemini 91%→92%），同时减少87.5%的推理token，降低成本69-82%。
...
### [67] [Beyond True or False: Retrieval-Augmented Hierarchical Analysis of Nuanced Claims](https://arxiv.org/abs/2506.10728)
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂声明（如科学或政治声明）难以简单标记为真或假的问题，提供更全面的视角。

Method: 提出ClaimSpect框架，通过层次化分解声明为多个子方面，并利用检索增强生成技术从语料库中提取相关视角。

Result: 在真实世界的科学和政治声明数据集上验证了其鲁棒性和准确性，优于多个基线方法。

Conclusion: ClaimSpect能有效解构复杂声明并展示语料库中的多角度观点，具有实用价值。
...
### [68] [TaxoAdapt: Aligning LLM-Based Multidimensional Taxonomy Construction to Evolving Research Corpora](https://arxiv.org/abs/2506.10737)
*Priyanka Kargupta,Nan Zhang,Yunyi Zhang,Rui Zhang,Prasenjit Mitra,Jiawei Han*

Main category: cs.CL

TL;DR: TaxoAdapt是一个动态调整LLM生成分类框架的多维方法，解决了传统分类方法在科学文献组织中的局限性。


<details>
  <summary>Details</summary>
Motivation: 科学领域的快速演变使得文献组织和检索变得困难，传统专家分类耗时且昂贵，现有自动分类方法缺乏通用性和动态适应性。

Method: TaxoAdapt通过迭代层次分类动态扩展分类的宽度和深度，适应多维度科学文献。

Result: TaxoAdapt在计算机科学会议数据集上表现优异，分类的粒度和连贯性分别提升26.51%和50.41%。

Conclusion: TaxoAdapt能有效捕捉科学领域的动态演变，生成更精细和连贯的分类体系。
...
### [69] [One Tokenizer To Rule Them All: Emergent Language Plasticity via Multilingual Tokenizers](https://arxiv.org/abs/2506.10766)
...
Method: 提出使用通用分词器，其覆盖的语言范围超过主要训练语言，以提升训练后的语言适应能力。

Result: 实验表明，通用分词器显著提升了语言适应能力，适应新语言的胜率最高提升20.2%，对完全未见语言的适应能力也有5%的提升。

Conclusion: 通用分词器设计是一种低成本且高效的方法，可在不显著影响主要训练语言性能的情况下，显著提升模型对新语言的适应能力。
...
### [70] [Different Questions, Different Models: Fine-Grained Evaluation of Uncertainty and Calibration in Clinical QA with LLMs](https://arxiv.org/abs/2506.10769)
...
<details>
  <summary>Details</summary>
Motivation: 在临床决策支持等高风险领域，大语言模型需要准确且校准良好的不确定性估计。

Method: 通过两个数据集、十一种医学专业和六种问题类型，比较了标准单生成和基于采样的方法，并探索了基于推理痕迹行为信号的轻量级方法。

Result: 轻量级方法在仅需一次生成的情况下接近语义熵的性能，且不同专业和问题类型间存在显著差异。
...
### [71] [Improving Named Entity Transcription with Contextual LLM-based Revision](https://arxiv.org/abs/2506.10779)
...
### [72] [Mitigating Negative Interference in Multilingual Sequential Knowledge Editing through Null-Space Constraints](https://arxiv.org/abs/2506.10800)
...
<details>
  <summary>Details</summary>
Motivation: 多语言大模型在更新知识时，跨语言的顺序编辑会导致参数干扰，影响多语言泛化能力和知识准确性。
...
### [73] [ReCUT: Balancing Reasoning Length and Accuracy in LLMs via Stepwise Trails and Preference Optimization](https://arxiv.org/abs/2506.10822)
...
<details>
  <summary>Details</summary>
Motivation: 现有CoT提示方法存在过度思考问题，导致冗长或冗余的推理轨迹，现有解决方案受限于生成数据质量和过拟合。

Method: ReCUT采用逐步探索机制和长短切换采样策略，生成多样化推理路径，训练两个专门模型（一个优化准确性，一个优化简短推理），最终通过参数插值集成。

Result: 在多个数学推理数据集和骨干模型上，ReCUT显著减少推理长度30-50%，同时保持或提高准确性。

Conclusion: ReCUT有效解决了推理冗长问题，平衡了准确性和简洁性，代码和数据已开源。
...
### [74] [CIIR@LiveRAG 2025: Optimizing Multi-Agent Retrieval Augmented Generation through Self-Training](https://arxiv.org/abs/2506.10844)
*Alireza Salemi,Mukta Maddipatla,Hamed Zamani*

Main category: cs.CL

TL;DR: mRAG是一个多代理检索增强生成框架，通过专门代理处理子任务（如规划、搜索、推理和协调），采用自训练范式优化代理协作并提升生成效果。
...
Method: 使用自训练范式与奖励引导的轨迹采样优化代理协作。
...
### [75] [Accelerating Diffusion Large Language Models with SlowFast: The Three Golden Principles](https://arxiv.org/abs/2506.10848)
...
<details>
  <summary>Details</summary>
Motivation: 现有的扩散语言模型采样策略存在静态行为问题，导致效率低下和灵活性不足。

Method: 提出SlowFast Sampling策略，基于确定性、收敛性和位置性三大原则，动态调整解码阶段，并结合dLLM-Cache优化计算。

Result: 实验表明，SlowFast Sampling在LLaDA上实现了15.63倍的速度提升，结合缓存后可达34.22倍，且准确率下降极小。

Conclusion: 该策略证明了通过优化采样设计，扩散语言模型可以实现高效高质量的生成，甚至超越传统自回归模型。
...
### [76] [Analyzing the relationships between pretraining language, phonetic, tonal, and speaker information in self-supervised speech models](https://arxiv.org/abs/2506.10855)
...
<details>
  <summary>Details</summary>
Motivation: 探索自监督语音模型在不同语言中的信息编码方式，弥补现有研究主要集中于英语的不足。

Method: 使用探测分类器和几何分析，研究四种语言训练的wav2vec2模型对语音、声调和说话者信息的编码。

Result: 所有语言中，语音、声调和说话者信息的子空间基本正交，层间探测准确率模式相似，仅在后期层中匹配语言的语音和声调探测略有优势。

Conclusion: wav2vec2学习的表示结构基本不受预训练语言材料影响。
...
### [77] [Enhancing Medical Dialogue Generation through Knowledge Refinement and Dynamic Prompt Adjustment](https://arxiv.org/abs/2506.10877)
...
<details>
  <summary>Details</summary>
Motivation: 现有医疗对话系统难以识别相关医学知识并生成个性化、准确的响应，因此需要改进。
...
### [78] [Slimming Down LLMs Without Losing Their Minds](https://arxiv.org/abs/2506.10885)
*Qingda,Mai*

Main category: cs.CL

TL;DR: 论文研究了参数高效方法（LoRA和QLoRA）对大型语言模型性能的影响，发现LoRA方法能有效提升任务性能并保持计算效率，性能与微调数据集和基准任务的匹配度密切相关。
...
Result: LoRA方法显著提升任务性能且计算高效，性能与数据集和任务的匹配度高度相关。
...
### [79] [Generalization or Hallucination? Understanding Out-of-Context Reasoning in Transformers](https://arxiv.org/abs/2506.10887)
*Yixiao Huang,Hanlin Zhu,Tianyu Guo,Jiantao Jiao,Somayeh Sojoudi,Michael I. Jordan,Stuart Russell,Song Mei*

Main category: cs.CL

TL;DR: 论文探讨了大语言模型（LLMs）通过微调获取新知识时的双重性：既能泛化新事实，又易产生幻觉。研究发现，这两种行为源于一种称为‘上下文外推理’（OCR）的机制，其表现取决于概念间是否存在因果关联。


<details>
  <summary>Details</summary>
Motivation: 理解LLMs在知识获取中表现出的泛化与幻觉并存现象的原因。

Method: 通过实验验证OCR机制的作用，并形式化OCR为合成事实回忆任务，分析单层单头注意力变换器在矩阵分解下的表现。

Result: OCR机制解释了LLMs的泛化与幻觉行为，矩阵分解是关键因素。梯度下降的隐式偏好最小化核范数，导致高效关联事实与推论。
...
### [80] [BioClinical ModernBERT: A State-of-the-Art Long-Context Encoder for Biomedical and Clinical NLP](https://arxiv.org/abs/2506.10896)
...
Result: 在四个下游任务中表现优于现有模型，发布基础版（150M参数）和大型版（396M参数）。

Conclusion: BioClinical ModernBERT通过多源数据和长上下文处理提升了生物医学和临床NLP的性能和适应性。
...
### [81] [Beyond Gold Standards: Epistemic Ensemble of LLM Judges for Formal Mathematical Reasoning](https://arxiv.org/abs/2506.10903)
*Lan Zhang,Marco Valentino,Andre Freitas*

Main category: cs.CL

TL;DR: 该论文提出了一种基于LLM的自动评估方法（EFG集成），用于评估自然语言到形式语言的自动形式化任务，解决了现有方法在高级数学领域中评估不足的问题。


<details>
  <summary>Details</summary>
Motivation: 自动形式化评估在高级数学领域中缺乏系统性和细粒度的方法，现有评估方法通常过于粗糙且通用，无法满足复杂形式化任务的需求。

Method: 提出了一种基于LLM的EFG集成评估方法，通过逻辑保持（LP）、数学一致性（MC）、形式有效性（FV）和形式质量（FQ）等多维度标准进行透明评估。

Result: 实验表明，EFG集成评估与人类评估的相关性更强，尤其在评估形式质量时表现优于粗糙模型。

Conclusion: LLM作为评估工具，在明确的原子属性指导下，可提供可扩展、可解释且可靠的自动形式化评估支持。
...
### [82] [Magistral](https://arxiv.org/abs/2506.10910)
...
Method: 采用自建模型和基础设施，通过纯RL训练LLM，提出强制模型推理语言的简单方法。

Result: RL训练保持或提升了多模态理解、指令遵循和函数调用能力。
...
### [83] [Decomposing MLP Activations into Interpretable Features via Semi-Nonnegative Matrix Factorization](https://arxiv.org/abs/2506.10920)
...
### [84] [Dynamic Epistemic Friction in Dialogue](https://arxiv.org/abs/2506.10934)
...
Conclusion: 动态认知摩擦模型为LLMs与人类协作中的信念对齐提供了新视角，未来可扩展以应对更复杂的对话场景。
...
### [85] [Domain2Vec: Vectorizing Datasets to Find the Optimal Data Mixture without Training](https://arxiv.org/abs/2506.10952)
...
<details>
  <summary>Details</summary>
Motivation: 当前语言模型预训练中，数据混合的选择对性能至关重要，但传统方法计算成本高且效率低。Domain2Vec旨在通过元域分解和数据分布对齐假设，以训练无关的方式优化数据混合。

Method: Domain2Vec通过分类器将数据集分解为元域向量（分布），利用分布对齐假设（DA²）找到最优数据混合，无需额外训练。

Result: 实验表明，Domain2Vec仅需51.5%的计算量即可达到与原数据混合相同的验证损失，并在相同计算预算下平均提升下游任务性能2.83%。

Conclusion: Domain2Vec提供了一种高效、可扩展的方法，显著优化语言模型预训练的数据混合选择，降低计算成本并提升性能。
...
### [86] [ChineseHarm-Bench: A Chinese Harmful Content Detection Benchmark](https://arxiv.org/abs/2506.10960)
...
Method: 构建了一个基于真实数据的专业标注中文有害内容检测基准，并提出了结合人工标注知识规则和大语言模型隐式知识的知识增强基线方法。

Result: 提出的知识增强方法使小模型能够达到与最先进大语言模型相当的性能。

Conclusion: 该研究为中文有害内容检测提供了宝贵的数据集和方法，推动了该领域的发展。
...
### [87] [AutoMind: Adaptive Knowledgeable Agent for Automated Data Science](https://arxiv.org/abs/2506.10974)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM代理框架依赖刚性工作流程和固定编码策略，难以应对复杂任务，缺乏人类专家的经验。

Method: AutoMind结合了专家知识库、树搜索算法和自适应编码策略，以动态适应任务复杂性。

Result: 在自动化数据科学基准测试中，AutoMind表现优于现有方法，展示了高效性和鲁棒性。

Conclusion: AutoMind是迈向完全自动化数据科学的重要一步，具有显著的性能和适应性优势。
...
TL;DR: 论文探讨了推理模型在识别和恢复四种无益思维方面的能力，发现模型能识别但难以恢复，且大模型表现更差。


<details>
  <summary>Details</summary>
Motivation: 研究推理模型如何有效进行自我重新评估，以提升准确性和安全性。

Method: 通过注入四种无益思维（如无关或误导性思维）测试模型的识别与恢复能力。
...
TL;DR: 该论文提出了一种利用Stable Diffusion、GPT-2和混合音频管道生成60秒电影视频的方法，结合了线性帧插值和后期处理，实现了高质量的视频合成。


<details>
  <summary>Details</summary>
Motivation: 生成式人工智能的进步改变了多媒体创作方式，本文旨在通过文本输入自动合成高质量的电影视频。

Method: 使用Stable Diffusion生成高保真图像，GPT-2构建叙事结构，混合音频管道（gTTS和YouTube音乐）生成声音，并通过五场景框架、帧插值和后期处理提升视频质量。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频编辑方法依赖大规模预训练，且缺乏对后续帧的灵活控制，限制了编辑的适应性。
...
TL;DR: DeepTraverse是一种受搜索算法启发的新型视觉架构，通过递归探索和自适应校准模块实现特征的系统性学习和优化，性能优于传统模型。


<details>
  <summary>Details</summary>
Motivation: 传统视觉主干网络的特征构建方式单一，缺乏自适应迭代优化的明确路径，因此探索是否可以通过搜索算法原则实现更结构化、逻辑化的处理流程。

Method: DeepTraverse采用递归探索模块（深化特征分析）和自适应校准模块（动态调整特征显著性），通过参数共享和全局上下文实现高效特征构建。
...
Conclusion: 该方法通过测试时适应显著提升了进度估计的泛化能力和准确性。
...
TL;DR: EfficientVLA是一个训练自由的推理加速框架，通过系统性消除冗余，显著提升Vision-Language-Action模型的效率。


<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action模型因计算和内存需求高而受限，现有加速方法未能全面解决其瓶颈。

Method: EfficientVLA采用三种策略：修剪语言模块冗余层、优化视觉处理路径、减少扩散动作头的计算冗余。

Result: 在CogACT模型上，推理速度提升1.93倍，FLOPs降至28.9%，成功率仅下降0.6%。

Conclusion: EfficientVLA通过系统性优化，显著提升了VLA模型的实用性和部署能力。
...
TL;DR: 论文介绍了首个多模态环境下检测未成年人内容的基准数据集ICCWD，包含1万张标注图像-标题对，并测试了三种检测方法，最高准确率为75.3%。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏多模态环境下检测未成年人内容的基准数据集，填补这一空白以支持更有效的未成年人内容识别工具开发。

Method: 发布ICCWD数据集，包含多样化的未成年人图像及标注，并测试了三种检测方法（包括商业年龄估计系统）。
...
TL;DR: RoCA是一种新的端到端自动驾驶框架，通过联合概率分布和Gaussian process提升跨域泛化能力，无需额外推理计算。
...
Method: RoCA通过联合概率分布建模车辆信息，利用Gaussian process学习基础token和轨迹，实现跨域泛化和适应。
...
TL;DR: 论文提出了一种名为SPARKE的方法，通过条件熵实现提示感知的多样性引导，解决了扩散模型在生成多样性上的挑战，并降低了计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成高保真图像和提示引导生成方面表现出色，但在生成样本的多样性上仍存在挑战，尤其是当提示语义广泛时。

Method: SPARKE方法利用条件熵进行多样性引导，动态测量相似提示的多样性，并通过优化计算复杂度从O(n^3)降至O(n)。

Result: 实验表明，SPARKE在多个文本到图像扩散模型中提高了生成数据的提示感知多样性，且未显著增加计算成本。

Conclusion: SPARKE方法有效解决了扩散模型在多样性生成上的问题，同时保持了计算效率。
...
TL;DR: 提出了一种基于注意力机制的模拟器，用于从卫星图像序列中学习晴空地表反射率，解决了山区动态雪覆盖导致的背景反射率估计问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖月度统计估计背景反射率，在山区因动态雪覆盖而失效，需要更灵活的方法。

Method: 采用Temporo-Spatial Vision Transformer，无需手工特征（如反照率图或云掩膜），直接从多光谱卫星图像序列中学习晴空反射率。
...
TL;DR: 本文提出了一种通过低秩适应调整视觉-语言双编码器模型的方法，用于个性化视觉-语言检索，并在两个基准测试中取得了优于现有方法4%-22%的性能。


<details>
  <summary>Details</summary>
Motivation: 个性化视觉-语言检索需要从少量示例中学习新概念（如“我的狗Fido”），并整合个人与通用知识以识别不同上下文中的概念。

Method: 采用语言编码器最后一层的小参数集的低秩适应方法，替代文本反转，同时保留通用知识。此外，探索了多个人概念参数组合的策略。

Result: 在DeepFashion2和ConCon-Chi两个基准测试中，个性化检索准确率比现有方法高出4%-22%。

Conclusion: 低秩适应方法在个性化视觉-语言检索中表现优异，既能学习新概念，又能保留通用知识。
...
Method: 在扩散采样过程中，通过凸组合混合不同类别的分数，生成合成样本，并研究类别选择策略。

Result: 在多个基准测试中显著提升判别器性能，发现混合判别器嵌入空间中距离较远的类别效果更佳。
...
TL;DR: 该研究提出了一个综合性的加州农作物产量基准数据集，结合多模态深度学习模型，实现了高精度的县级作物产量预测。
...
Result: 模型在未见测试数据上达到R2分数0.76，表现出强大的预测性能。
...
### [104] [DySS: Dynamic Queries and State-Space Learning for Efficient 3D Object Detection from Multi-Camera Videos](https://arxiv.org/abs/2506.10242)
*Rajeev Yasarla,Shizhong Han,Hong Cai,Fatih Porikli*

Main category: cs.CV

TL;DR: DySS是一种基于状态空间学习和动态查询的3D目标检测方法，通过稀疏查询和动态更新操作实现高效推理和优越性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖密集BEV特征或大量查询，计算成本高且效率低，DySS旨在通过动态查询和状态空间学习解决这一问题。

Method: DySS利用状态空间模型（SSM）处理时序特征，引入未来预测和掩码重建任务优化SSM，并通过动态更新查询（合并、移除、拆分）保持高效检测。

Result: 在nuScenes测试集上，DySS达到65.31 NDS和57.4 mAP，优于现有方法；验证集上为56.2 NDS和46.2 mAP，推理速度为33 FPS。

Conclusion: DySS在性能和效率上均表现优异，为自动驾驶感知任务提供了高效解决方案。
...
### [105] [HalLoc: Token-level Localization of Hallucinations for Vision Language Models](https://arxiv.org/abs/2506.10286)
*Eunkyu Park,Minyeong Kim,Gunhee Kim*

Main category: cs.CV

TL;DR: HalLoc是一个用于高效、概率性幻觉检测的数据集，包含150K标记级注释样本，支持视觉问答、指令跟随和图像描述任务。
...
Result: HalLoc数据集和基线模型为幻觉检测提供了高效解决方案，并支持分级置信度检测。

Conclusion: HalLoc为提升视觉语言模型在真实应用中的可靠性提供了新途径。
...
### [106] [Uncertainty-Aware Deep Learning for Automated Skin Cancer Classification: A Comprehensive Evaluation](https://arxiv.org/abs/2506.10302)
...
Method: 使用迁移学习和不确定性量化（UQ）评估多种预训练特征提取器和分类器，并引入UQ方法（MCD、Ensemble、EMCD）。

Result: CLIP-based视觉变换器（如LAION CLIP ViT-H/14与SVM结合）性能最高；集成方法在准确性和不确定性处理间表现良好。
...
### [107] [Towards Scalable SOAP Note Generation: A Weakly Supervised Multimodal Framework](https://arxiv.org/abs/2506.10328)
*Sadia Kamal,Tim Oates,Joy Wan*

Main category: cs.CV

TL;DR: 提出了一种弱监督多模态框架，用于从有限的输入（如病变图像和稀疏临床文本）生成临床结构化的SOAP笔记，减少对人工标注的依赖，性能媲美主流模型。


<details>
  <summary>Details</summary>
Motivation: 皮肤癌是全球最常见的癌症，手动生成SOAP笔记耗时且易导致医生疲劳，需要自动化解决方案。
...
### [108] [Research on Audio-Visual Quality Assessment Dataset and Method for User-Generated Omnidirectional Video](https://arxiv.org/abs/2506.10331)
*Fei Zhao,Da Pan,Zelu Qi,Ping Shi*

Main category: cs.CV

TL;DR: 论文针对元宇宙中用户生成的全向视频（ODV）的音视频质量评估（AVQA）问题，构建了一个数据集并提出了一个基线模型。
...
Method: 构建了一个UGC全向音视频数据集，包含300个视频，覆盖10种场景类型，并通过主观实验获得平均意见分数（MOS）。随后提出了一个基线模型，包括视频特征提取、音频特征提取和音视频融合模块。
...
### [109] [Using Vision Language Models to Detect Students' Academic Emotion through Facial Expressions](https://arxiv.org/abs/2506.10334)
*Deliang Wang,Chao Yang,Gaowei Chen*

Main category: cs.CV

TL;DR: 研究探讨了视觉语言模型（VLMs）在在线学习环境中通过零样本提示分析学生学术情绪（如困惑、分心、快乐等）的潜力，初步结果显示Qwen2.5-VL-7B-Instruct表现优于Llama-3.2-11B-Vision-Instruct。


<details>
  <summary>Details</summary>
Motivation: 传统监督学习方法难以泛化到不同情境，而VLMs通过零样本提示提供了一种无需微调的替代方案，有望更高效地分析学生情绪。

Method: 使用Llama-3.2-11B-Vision-Instruct和Qwen2.5-VL-7B-Instruct两种VLMs，通过零样本提示分析5000张包含不同情绪（困惑、分心、快乐等）的学生面部图像。

Result: Qwen2.5-VL-7B-Instruct表现更优，尤其在识别快乐和困惑情绪上，但两种模型均无法有效检测分心行为。
...
### [110] [PointGS: Point Attention-Aware Sparse View Synthesis with Gaussian Splatting](https://arxiv.org/abs/2506.10335)
...
Method: 利用立体基础模型估计相机位姿并重建密集点云；通过多尺度2D特征采样和自注意力网络增强点表示；用轻量MLP解码高斯参数。

Result: 在多样基准测试中显著优于NeRF方法，在少样本设置下与最先进3DGS方法竞争。

Conclusion: 该方法有效提升了稀疏视图下的渲染质量和鲁棒性。
...
### [111] [GeoCAD: Local Geometry-Controllable CAD Generation](https://arxiv.org/abs/2506.10337)
*Zhanwei Zhang,Kaiyuan Liu,Junjie Liu,Wenxiao Wang,Binbin Lin,Liang Xie,Chen Shen,Deng Cai*

Main category: cs.CV

TL;DR: GeoCAD是一种用户友好且局部几何可控的CAD生成方法，通过互补标注策略生成局部几何指令，利用LLM预测被遮蔽部分，实验证明其有效性。
...
Method: 提出互补标注策略（顶点和VLLM标注），训练时随机遮蔽局部部分，利用几何指令和剩余部分输入LLM预测。
...
### [112] [UrbanSense:AFramework for Quantitative Analysis of Urban Streetscapes leveraging Vision Large Language Models](https://arxiv.org/abs/2506.10342)
*Jun Yin,Jing Zhong,Peilin Li,Pengyu Zeng,Miao Zhang,Ran Luo,Shuai Lu*

Main category: cs.CV

TL;DR: 提出了一种基于视觉语言模型的多模态研究框架，用于自动化分析城市街景风格差异，并构建了UrbanDiffBench数据集和UrbanSense框架。
...
Method: 采用视觉语言模型构建多模态框架，自动化分析街景风格差异，并开发UrbanSense框架和UrbanDiffBench数据集。

Result: 实验结果显示，超过80%的描述通过t检验，主观评价的高Phi分数（城市0.912，时期0.833）验证了方法的有效性。

Conclusion: 该方法能量化城市风格演变，为未来设计提供科学依据。
...
### [113] [RealKeyMorph: Keypoints in Real-world Coordinates for Resolution-agnostic Image Registration](https://arxiv.org/abs/2506.10344)
...
Result: 实验证明RKM在腹部MRI正交2D堆栈和不同分辨率脑数据集3D配准任务中表现优异。
...
### [114] [Motion-R1: Chain-of-Thought Reasoning and Reinforcement Learning for Human Motion Generation](https://arxiv.org/abs/2506.10353)
*Runqi Ouyang,Haoyun Li,Zhenyuan Zhang,Xiaofeng Wang,Zheng Zhu,Guan Huang,Xingang Wang*

Main category: cs.CV

TL;DR: Motion-R1是一个结合Chain-of-Thought机制的统一运动-语言建模框架，通过分解文本指令为逻辑动作路径，提升运动生成的可控性和多样性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到运动生成方法依赖端到端映射，缺乏对深层语言结构和逻辑推理的捕捉，导致生成的运动缺乏可控性和一致性。
...
### [115] [FaceLiVT: Face Recognition using Linear Vision Transformer with Structural Reparameterization For Mobile Device](https://arxiv.org/abs/2506.10361)
*Novendra Setyawan,Chi-Chia Sun,Mao-Hsiu Hsu,Wen-Kai Kuo,Jun-Wei Hsieh*

Main category: cs.CV

TL;DR: FaceLiVT是一种轻量级但强大的人脸识别模型，结合了CNN-Transformer架构和创新的多头部线性注意力机制（MHLA），显著降低计算复杂度并保持高精度。
...
Method: 采用混合CNN-Transformer架构和创新的MHLA机制，结合重新参数化的token mixer，优化计算效率。
...
### [116] [FSATFusion: Frequency-Spatial Attention Transformer for Infrared and Visible Image Fusion](https://arxiv.org/abs/2506.10366)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法使用卷积神经网络提取特征，但卷积操作难以捕捉全局上下文，导致信息丢失，限制了融合效果。

Method: 设计了FSAT模块（包含频率-空间注意力机制FSAM）和ITM模块，分别用于提取显著特征和增强全局上下文信息。

Result: 实验表明FSATFusion在融合质量和效率上优于其他先进方法，并展示了出色的泛化能力和在下游视觉任务中的优势。

Conclusion: FSATFusion通过创新的模块设计解决了现有方法的局限性，具有广泛的应用潜力。
...
### [117] [Revisiting Transformers with Insights from Image Filtering](https://arxiv.org/abs/2506.10371)
*Laziz U. Abdullaev,Maksim Tkachenko,Tan M. Nguyen*

Main category: cs.CV

TL;DR: 本文通过图像处理框架解释自注意力机制及其组件（如位置编码和残差连接），并提出两种改进方法，既增强可解释性又提升模型性能。
...
Conclusion: 图像处理框架为自注意力机制提供理论支持，改进方法兼具解释性和性能优势。
...
### [118] [Leveraging 6DoF Pose Foundation Models For Mapping Marine Sediment Burial](https://arxiv.org/abs/2506.10386)
*Jerry Yan,Chinmay Talegaonkar,Nicholas Antipa,Eric Terrill,Sophia Merrifield*

Main category: cs.CV

TL;DR: 论文提出了一种名为PoseIDON的计算机视觉方法，通过结合深度基础模型特征和多视角摄影测量技术，从ROV视频中估计海底物体的六自由度位姿及周围海底的朝向，从而推断埋藏深度。该方法在历史海洋倾倒场的54个物体上验证，平均埋藏深度误差约为10厘米。


<details>
  <summary>Details</summary>
Motivation: 准确估计海底人为物体的埋藏状态对于理解局部沉积动态、评估生态风险、污染物迁移以及危险材料（如弹药）的回收或缓解策略至关重要。然而，由于部分遮挡、能见度差和物体退化，从遥感图像中准确估计埋藏深度仍然具有挑战性。

Method: 论文提出了一种计算机视觉流程PoseIDON，结合深度基础模型特征与多视角摄影测量技术，从ROV视频中估计物体的六自由度位姿和周围海底的朝向。埋藏深度通过将物体的CAD模型与观测图像对齐，并拟合局部平面近似海底来推断。

Result: 在San Pedro Basin历史海洋倾倒场的54个物体（包括桶和弹药）上验证，模型平均埋藏深度误差约为10厘米，并能解析反映底层沉积物迁移过程的空间埋藏模式。
...
### [119] [DART: Differentiable Dynamic Adaptive Region Tokenizer for Vision Transformer and Mamba](https://arxiv.org/abs/2506.10390)
...
Method: DART结合可学习区域评分和分段可微分分位数操作，自适应分配更密集的标记到信息丰富区域，仅增加约1M参数。

Result: 在DeiT（ImageNet-1K）上准确率提升2.1%，FLOPs减少45%，性能优于均匀增加标记密度的方法。

Conclusion: DART在DeiT、Vim和VideoMamba上一致提升准确性，计算开销最小甚至减少，代码已开源。
...
### [120] [ReconMOST: Multi-Layer Sea Temperature Reconstruction with Observations-Guided Diffusion](https://arxiv.org/abs/2506.10391)
*Yuanyi Song,Pumeng Lyu,Ben Fei,Fenghua Ling,Wanli Ouyang,Lei Bai*

Main category: cs.CV

TL;DR: 论文提出ReconMOST框架，利用数据驱动的扩散模型进行多层海水温度重建，解决了传统方法在稀疏数据和高计算成本上的问题，并在全球范围内实现了高精度重建。


<details>
  <summary>Details</summary>
Motivation: 准确重建海洋温度对全球气候动力学和海洋气象研究至关重要，但传统方法因数据稀疏、算法复杂和计算成本高而受限，机器学习方法也仅适用于表层或局部区域。

Method: 通过预训练无条件扩散模型学习历史数值模拟数据的物理一致性分布，再以高精度观测数据为引导点进行反向扩散，实现多层温度重建。

Result: 在CMIP6和EN4数据上的实验显示，MSE分别为0.049（引导）、0.680（重建）和0.633（总体），证明了框架的有效性和鲁棒性。

Conclusion: ReconMOST扩展了机器学习在海洋温度重建中的应用，实现了全球多层高精度重建，并具备优异的泛化能力。
...
### [121] [Pisces: An Auto-regressive Foundation Model for Image Understanding and Generation](https://arxiv.org/abs/2506.10395)
...
Method: 提出Pisces模型，采用解耦视觉编码架构和定制化训练技术，结合精细数据整理、预训练和微调。

Result: 在20多个图像理解基准测试中表现优异，并在图像生成基准测试GenEval中展现强大生成能力。

Conclusion: Pisces展示了图像理解与生成之间的协同关系，解耦视觉编码器的优势，推动了统一多模态模型的发展。
...
### [122] [It's Not the Target, It's the Background: Rethinking Infrared Small Target Detection via Deep Patch-Free Low-Rank Representations](https://arxiv.org/abs/2506.10425)
*Guoyi Zhang,Guangsheng Xu,Siyang Chen,Han Wang,Xiaohu Zhang*

Main category: cs.CV

TL;DR: 论文提出了一种名为LRRNet的新型端到端红外小目标检测框架，利用红外图像背景的低秩特性，通过压缩-重建-减法（CRS）范式直接建模结构感知的低秩背景表示，显著提升了检测精度和实时性能。


<details>
  <summary>Details</summary>
Motivation: 红外小目标检测（IRSTD）在复杂背景下面临低信噪比、目标形态多样和缺乏视觉线索的挑战，现有深度学习方法因目标内在变异性导致性能不稳定。
...
Conclusion: LRRNet首次通过端到端深度网络直接学习低秩背景结构，为IRSTD提供了高效且鲁棒的解决方案。
...
### [123] [MF2Summ: Multimodal Fusion for Video Summarization with Temporal Alignment](https://arxiv.org/abs/2506.10430)
...
<details>
  <summary>Details</summary>
Motivation: 传统视频摘要方法通常仅依赖单一模态（如视觉），难以捕捉视频的完整语义丰富性。

Method: MF2Summ采用五阶段流程：特征提取、跨模态注意力交互、特征融合、片段预测和关键镜头选择，使用GoogLeNet和SoundNet提取特征，并通过Transformer建模模态间依赖关系。

Result: 在SumMe和TVSum数据集上，MF2Summ的F1分数分别比DSNet模型提高了1.9%和0.6%，优于其他先进方法。

Conclusion: MF2Summ通过多模态融合显著提升了视频摘要的性能，验证了跨模态方法的重要性。
...
### [124] [Towards Robust Multimodal Emotion Recognition under Missing Modalities and Distribution Shifts](https://arxiv.org/abs/2506.10452)
*Guowei Zhong,Ruohong Huan,Mingzhen Wu,Ronghua Liang,Peng Chen*

Main category: cs.CV

TL;DR: 论文提出了一种名为CIDer的鲁棒多模态情感识别框架，通过自蒸馏和因果推理模块解决模态缺失和分布外数据问题，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多模态情感识别面临模态缺失和分布外数据的双重挑战，现有方法实用性受限。

Method: 提出CIDer框架，包含自蒸馏模块（MSSD）和因果推理模块（MACI），并引入新任务RMFM。

Result: CIDer在RMFM和OOD场景中表现鲁棒，参数更少且训练更快。

Conclusion: CIDer有效解决了模态缺失和OOD问题，性能优越且实用性强。
...
### [125] [Rethinking Generative Human Video Coding with Implicit Motion Transformation](https://arxiv.org/abs/2506.10453)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于显式运动的生成式视频编码在人体视频压缩中因复杂多样的运动模式导致重建结果失真和运动不准确，因此需要改进方法。

Method: 通过将复杂人体信号表征为紧凑视觉特征，并将这些特征转化为隐式运动引导以重建信号。

Result: 实验证明IMT范式能有效提升GHVC的高效压缩和高保真合成性能。

Conclusion: 隐式运动变换（IMT）显著改善了生成式人体视频编码的性能，适用于复杂运动场景。
...
### [126] [Boosting Adversarial Transferability for Hyperspectral Image Classification Using 3D Structure-invariant Transformation and Intermediate Feature Distance](https://arxiv.org/abs/2506.10459)
*Chun Liu,Bingqian Zhu,Tao Xu,Zheng Zheng,Zheng Li,Wei Yang,Zhigang Han,Jiayao Wang*

Main category: cs.CV

TL;DR: 本文提出了一种增强高光谱图像分类模型对抗样本可迁移性的新方法，通过随机分块和特征距离损失设计，显著提升了对抗样本在黑盒模型中的攻击效果。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像（HSI）因其高维和丰富的光谱信息与自然图像不同，现有对抗攻击方法在HSI上的研究有限，且未能充分利用图像结构和特征信息。

Method: 方法包括随机分块以增加输入多样性，并设计特征距离损失，以中间层特征距离为主损失、输出层预测为辅助损失，引导扰动破坏真实类特征。

Result: 实验表明，生成的对抗样本在公开HSI数据集上对黑盒模型具有高可迁移性，且在防御策略下仍保持强攻击性能。
...
### [127] [Starting Positions Matter: A Study on Better Weight Initialization for Neural Network Quantization](https://arxiv.org/abs/2506.10463)
...
<details>
  <summary>Details</summary>
Motivation: 量化是降低机器学习模型推理成本的重要工具，但现有研究很少关注权重初始化对量化鲁棒性的影响。论文旨在探索不同初始化方法对量化模型的影响，并提出改进方案。

Method: 论文首先分析了不同权重初始化方法对CNN量化鲁棒性的影响，随后提出GHN-QAT方法，利用图超网络预测量化DNN的参数，并通过微调进一步提升量化精度。

Result: 实验表明，GHN-QAT显著提升了4位量化的精度，并在2位量化中表现优于随机初始化。这是首次对量化感知的DNN权重初始化进行深入研究。
...
### [128] [MedSeg-R: Reasoning Segmentation in Medical Images with Multimodal Large Language Models](https://arxiv.org/abs/2506.10465)
*Yu Huang,Zelin Peng,Yichen Zhao,Piao Yang,Xiaokang Yang,Wei Shen*

Main category: cs.CV

TL;DR: 论文提出了一种新的医学图像分割任务（MedSeg-R），结合多模态大语言模型（MLLMs）的推理能力，通过全局上下文理解和像素级定位模块生成精确的分割掩码和文本响应。同时，还发布了MedSeg-QA数据集，实验证明了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割模型依赖显式人类指令，缺乏主动推理能力，难以处理复杂临床问题。多模态大语言模型在医学问答任务中表现优异，但生成精确分割掩码的能力有限。

Method: 提出MedSeg-R框架，包含全局上下文理解模块（生成多模态中间标记）和像素级定位模块（解码标记生成分割掩码和文本响应）。

Result: MedSeg-R在多个基准测试中表现优异，实现了高分割精度和可解释的医学图像文本分析。

Conclusion: MedSeg-R通过结合MLLMs的推理能力，解决了复杂医学指令下的图像分割问题，为自动医学诊断提供了新思路。
...
### [129] [LLMs Are Not Yet Ready for Deepfake Image Detection](https://arxiv.org/abs/2506.10474)
*Shahroz Tariq,David Nguyen,M. A. P. Chamikara,Tingmin Wu,Alsharif Abuadbba,Kristen Moore*

Main category: cs.CV

TL;DR: 本文评估了四种视觉语言模型（VLM）在零样本设置下对三类深度伪造（deepfake）的检测能力，发现其虽能生成合理解释但尚不可靠，但可作为辅助工具。


<details>
  <summary>Details</summary>
Motivation: 深度伪造技术日益复杂，威胁媒体诚信和公众信任，而视觉语言模型（VLM）在多领域表现出潜力，因此研究其在深度伪造检测中的适用性。

Method: 通过结构化零样本评估四种VLM（ChatGPT、Claude、Gemini、Grok），使用包含真实和伪造图像的基准数据集，评估分类准确性和推理深度。

Result: VLM能生成合理解释并检测表面异常，但不可靠；易受误导性视觉模式（如复古风格）影响，但在可解释性和上下文分析方面表现突出。

Conclusion: 通用VLM目前不适合独立检测深度伪造，但在混合或人机协作框架中具有潜力。
...
### [130] [Sheet Music Benchmark: Standardized Optical Music Recognition Evaluation](https://arxiv.org/abs/2506.10488)
...
Method: 创建SMB数据集和OMR-NED指标，基于Humdrum **kern格式，涵盖多种音乐纹理。

Result: OMR-NED提供了细粒度错误分析，支持清晰的性能比较。
...
### [131] [Class-Incremental Learning for Honey Botanical Origin Classification with Hyperspectral Images: A Study with Continual Backpropagation](https://arxiv.org/abs/2506.10489)
...
Result: 实验显示CB方法能提高大多数CIL算法的性能1-7%。

Conclusion: 结合CB的CIL技术能有效解决蜂蜜植物源区分问题，提升模型性能。
...
### [132] [Semantic Localization Guiding Segment Anything Model For Reference Remote Sensing Image Segmentation](https://arxiv.org/abs/2506.10503)
...
Conclusion: PSLG-SAM有效解决了RRSIS任务中的问题，并提供了高质量数据集。
...
### [133] [J-DDL: Surface Damage Detection and Localization System for Fighter Aircraft](https://arxiv.org/abs/2506.10505)
...
<details>
  <summary>Details</summary>
Motivation: 战斗机表面检测需频繁且全面，但人工方法在可扩展性、效率和一致性上存在局限，亟需自动化解决方案。

Method: 集成激光扫描仪与相机捕获2D图像和3D点云，采用优化的YOLO架构（含Fasternet块、EMA模块和Inner-CIOU损失函数）检测缺陷，并映射至3D点云定位。

Result: 实验验证J-DDL能高效检测并准确定位表面缺陷，提升检测覆盖率和自动化水平。

Conclusion: J-DDL为战斗机表面检测提供高效自动化方案，并公开首个飞机损伤数据集，推动该领域发展。
...
### [134] [CogStream: Context-guided Streaming Video Question Answering](https://arxiv.org/abs/2506.10516)
*Zicheng Zhao,Kangyu Wang,Shijie Li,Rui Qian,Weiyao Lin,Huabin Liu*

Main category: cs.CV

TL;DR: 论文提出了一种名为CogStream的新任务，用于解决流媒体视频推理中的上下文依赖问题，并提出了一个基线模型CogReasoner。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型（Vid-LLMs）在处理流媒体视频时依赖全部历史上下文信息，导致计算负担重且容易受无关信息干扰。
...
### [135] [ALBERT: Advanced Localization and Bidirectional Encoder Representations from Transformers for Automotive Damage Evaluation](https://arxiv.org/abs/2506.10524)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够智能检测和评估汽车损伤的模型，以支持汽车检查和评估应用。
...
### [136] [SLICK: Selective Localization and Instance Calibration for Knowledge-Enhanced Car Damage Segmentation in Automotive Insurance](https://arxiv.org/abs/2506.10528)
*Teerapong Panboonyuen*
...
### [137] [ContextRefine-CLIP for EPIC-KITCHENS-100 Multi-Instance Retrieval Challenge 2025](https://arxiv.org/abs/2506.10550)
...
### [138] [From Images to Insights: Explainable Biodiversity Monitoring with Plain Language Habitat Explanations](https://arxiv.org/abs/2506.10559)
*Yutong Zhou,Masahiro Ryo*

Main category: cs.CV

TL;DR: 提出了一种端到端的视觉到因果框架，通过图像分析生成物种栖息地偏好的可解释因果洞察，整合了物种识别、全球分布检索、伪缺失采样和气候数据提取，并利用因果推断方法生成人类可读的解释。
...
Conclusion: 该框架为生态学家和非专家提供了一种直观且可解释的工具，有助于理解物种栖息地偏好。
...
### [139] [Balancing Tails when Comparing Distributions: Comprehensive Equity Index (CEI) with Application to Bias Evaluation in Operational Face Biometrics](https://arxiv.org/abs/2506.10564)
...
Method: 引入CEI指标，分别分析真实和冒名分数分布，可配置地关注尾部概率，同时考虑整体分布形状。还提出了自动版本CEI^A。

Result: 实验表明，CEI在检测细微偏差方面优于现有方法，尤其在尾部概率分析中表现突出。

Conclusion: CEI为评估人脸识别系统的公平性提供了敏感且稳健的工具，适用于任何需要分析分布尾部的统计问题。
...
### [140] [LRSLAM: Low-rank Representation of Signed Distance Fields in Dense Visual SLAM System](https://arxiv.org/abs/2506.10567)
*Hongbeen Park,Minjeong Park,Giljoo Nam,Jinkyu Kim*

Main category: cs.CV

TL;DR: LRSLAM提出了一种基于低秩张量分解的高效视觉SLAM方法，解决了现有方法在实时性、内存占用和扩展性上的问题。
...
### [141] [DreamActor-H1: High-Fidelity Human-Product Demonstration Video Generation via Motion-designed Diffusion Transformers](https://arxiv.org/abs/2506.10568)
*Lizhen Wang,Zhurong Xia,Tianshu Hu,Pengrui Wang,Pengfei Wang,Zerong Zheng,Ming Zhou*

Main category: cs.CV

TL;DR: 提出了一种基于扩散变换器（DiT）的框架，用于生成高质量的人与产品演示视频，解决了现有方法在身份保留和空间关系理解上的不足。


<details>
  <summary>Details</summary>
Motivation: 在电子商务和数字营销中，生成高保真的人与产品演示视频对产品展示至关重要，但现有方法难以同时保留人和产品的身份或理解其空间关系。

Method: 采用DiT框架，通过注入配对的人与产品参考信息和使用掩码交叉注意力机制，同时保留人和产品的身份细节。使用3D身体网格模板和产品边界框提供精确运动指导，并结合结构化文本编码增强3D一致性。
...
### [142] [Improving Medical Visual Representation Learning with Pathological-level Cross-Modal Alignment and Correlation Exploration](https://arxiv.org/abs/2506.10573)
*Jun Wang,Lixing Zhu,Xiaohan Yu,Abhir Bhalerao,Yulan He*

Main category: cs.CV

TL;DR: PLACE框架通过病理级对齐和相关性探索，提升医学视觉表示学习，无需额外标注，在多项下游任务中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 解决医学领域数据稀缺问题，同时应对报告冗长、语义复杂和病理级一致性被忽视的挑战。

Method: 提出病理级跨模态对齐（PCMA）方法，结合视觉病理观察提取器和相关性代理任务，增强细粒度细节。

Result: 在分类、图像到文本检索、语义分割、目标检测和报告生成等任务中达到最新最优性能。

Conclusion: PLACE框架通过病理级对齐和相关性探索，显著提升医学视觉表示学习的性能，具有广泛适用性。
...
### [143] [DanceChat: Large Language Model-Guided Music-to-Dance Generation](https://arxiv.org/abs/2506.10574)
*Qing Wang,Xiaohang Yang,Yilan Dong,Naveen Raj Govindaraj,Gregory Slabaugh,Shanxin Yuan*

Main category: cs.CV

TL;DR: DanceChat利用大型语言模型（LLM）生成文本舞蹈指导，结合音乐风格和结构，通过多模态特征融合和扩散模型生成多样且与音乐风格一致的舞蹈动作。


<details>
  <summary>Details</summary>
Motivation: 音乐与舞蹈之间存在语义鸿沟，音乐仅提供抽象线索（如旋律、节奏、情感），无法明确指定具体动作。此外，一对多的映射关系和数据稀缺性增加了生成多样性舞蹈的难度。

Method: 1. LLM生成伪指令；2. 多模态特征提取与融合；3. 扩散模型合成动作并多模态对齐。

Result: 在AIST++数据集和人工评估中，DanceChat在质量和多样性上均优于现有方法。

Conclusion: DanceChat通过LLM提供显式指导，解决了音乐到舞蹈生成的多样性和对齐问题，表现优于现有技术。
...
### [144] [Text to Image for Multi-Label Image Recognition with Joint Prompt-Adapter Learning](https://arxiv.org/abs/2506.10575)
*Chun-Mei Feng,Kai Yu,Xinxing Xu,Salman Khan,Rick Siow Mong Goh,Wangmeng Zuo,Yong Liu*

Main category: cs.CV

TL;DR: T2I-PAL利用文本生成图像减少模态差异，结合热图和原型学习提升多标签识别性能，无需全标注图像，显著提升CLIP模型效果。
...
Method: 利用文本生成图像模型生成多样图像，结合类热图和可学习原型增强局部特征表示，同时结合提示调优和适配器学习。
...
### [145] [Harmonizing Geometry and Uncertainty: Diffusion with Hyperspheres](https://arxiv.org/abs/2506.10576)
...
<details>
  <summary>Details</summary>
Motivation: 标准扩散模型基于各向同性高斯噪声，适用于欧几里得空间，但无法有效处理非欧几里得分布（如超球面流形）中的角几何特征。

Method: 引入HyperSphereDiff，利用方向性噪声保留类几何特征，并捕捉角不确定性。

Result: 理论和实验证明，该方法能更好地对齐超球面数据的本征几何，生成更准确的几何感知模型。

Conclusion: HyperSphereDiff在多个数据集上验证了其有效性，显著提升了超球面流形的生成性能。
...
### [146] [Rethinking Random Masking in Self Distillation on ViT](https://arxiv.org/abs/2506.10582)
...
Conclusion: 非对称随机掩码策略在自蒸馏框架中有效，既能保留关键信息，又能增强模型鲁棒性。
...
### [147] [Hierarchical Error Assessment of CAD Models for Aircraft Manufacturing-and-Measurement](https://arxiv.org/abs/2506.10594)
...
Method: 使用结构光扫描仪获取工件3D测量数据，通过点云配准和分层误差分析（全局、部件、特征），提出基于优化的基元细化方法和两阶段圆形孔检测算法。

Result: 实验结果表明，该方法在多种飞机CAD模型上具有有效性。

Conclusion: HEA-MM框架为航空制造中的误差评估提供了一种高效且精确的解决方案。
...
TL;DR: SSP框架通过语义解耦空间分区，解决了点监督下定向目标检测的样本分配和实例混淆问题，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 高密度场景中定向目标检测的标注成本高，现有方法因规则设计刚性导致样本分配不足和实例混淆。

Method: SSP结合规则驱动的先验注入和数据驱动的标签净化，提出像素级和语义级空间分区方法，优化样本分配和伪标签生成。

Result: 在DOTA-v1.0等数据集上，SSP的mAP达45.78%，优于PointOBB-v2 4.10%；与ORCNN和ReDet结合后分别达47.86%和48.50%。

Conclusion: SSP为高密度场景的定向目标检测提供了高效解决方案，显著提升了点监督下的性能。
...
TL;DR: LatentCSI是一种利用预训练潜在扩散模型（LDM）从WiFi CSI测量生成物理环境图像的新方法，相比传统GANs更高效且支持文本引导控制。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖复杂且计算密集的技术（如GANs），难以高效生成高质量图像。LatentCSI旨在通过轻量级网络直接映射CSI到LDM潜在空间，简化流程并提升效果。

Method: 使用轻量级神经网络将CSI幅度直接映射到LDM潜在空间，结合文本引导的扩散模型去噪，最后通过LDM解码器生成高分辨率图像。
...
### [150] [MSTAR: Box-free Multi-query Scene Text Retrieval with Attention Recycling](https://arxiv.org/abs/2506.10609)
*Liang Yin,Xudong Xie,Zhang Li,Xiang Bai,Yuliang Liu*

Main category: cs.CV

TL;DR: MSTAR是一种无需边界框标注的场景文本检索方法，通过动态多粒度文本表示和风格感知指令统一多种查询类型，显著提升了检索性能。
...
Conclusion: MSTAR通过消除标注成本并统一查询类型，显著推动了场景文本检索的发展。
...
### [151] [TexTailor: Customized Text-aligned Texturing via Effective Resampling](https://arxiv.org/abs/2506.10612)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在纹理合成过程中存在视角间纹理属性逐渐偏移的问题，且相机位置选择未考虑物体几何形状，导致纹理一致性差。

Method: TexTailor采用重采样方案整合先前合成的纹理信息，并微调深度感知扩散模型，同时提出性能保留损失和自适应相机位置调整。
...
### [152] [Anatomy-Grounded Weakly Supervised Prompt Tuning for Chest X-ray Latent Diffusion Models](https://arxiv.org/abs/2506.10633)
...
Result: 在标准数据集（MS-CXR）上达到最优性能，并在外部数据（VinDr-CXR）上表现出鲁棒性。

Conclusion: 该方法为医学影像领域的文本到图像生成任务提供了有效解决方案，并具有广泛的应用潜力。
...
### [153] [Symmetrical Flow Matching: Unified Image Generation, Segmentation, and Classification with Score-Based Generative Models](https://arxiv.org/abs/2506.10634)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在分布转换中缺乏灵活性和双向一致性，SymmFlow旨在解决这些问题，同时支持多任务学习。

Method: 采用对称学习目标，联合建模正向和反向转换，引入新训练目标以保留语义信息，支持像素级和图像级标签。

Result: 在CelebAMask-HQ和COCO-Stuff上分别取得FID 11.9和7.0的SOTA性能，同时在分割和分类任务中表现优异。

Conclusion: SymmFlow展示了多任务学习的潜力，为生成模型和语义任务提供了统一框架。
...
### [154] [GigaVideo-1: Advancing Video Generation via Automatic Feedback with 4 GPU-Hours Fine-Tuning](https://arxiv.org/abs/2506.10639)
*Xiaoyi Bao,Jindi Lv,Xiaofeng Wang,Zheng Zhu,Xinze Chen,YuKun Zhou,Jiancheng Lv,Xingang Wang,Guan Huang*

Main category: cs.CV

TL;DR: GigaVideo-1是一种无需人工监督的高效视频生成微调框架，通过自动反馈提升预训练模型的性能。
...
Conclusion: GigaVideo-1无需人工标注和大量真实数据，高效且有效。
...
### [155] [PiPViT: Patch-based Visual Interpretable Prototypes for Retinal Image Analysis](https://arxiv.org/abs/2506.10669)
*Marzieh Oghbaie,Teresa Araújoa,Hrvoje Bogunović*

Main category: cs.CV

TL;DR: PiPViT是一种基于视觉Transformer的原型模型，通过学习可解释的病灶原型，提升医学图像的分类性能和解释性。
...
### [156] [Enhancing Deepfake Detection using SE Block Attention with CNN](https://arxiv.org/abs/2506.10683)
*Subhram Dasgupta,Janelle Mason,Xiaohong Yuan,Olusola Odeyomi,Kaushik Roy*

Main category: cs.CV

TL;DR: 提出了一种轻量级CNN结合SE模块的深度伪造检测模型，解决了传统模型存储和内存消耗大的问题，实现了高效检测。


<details>
  <summary>Details</summary>
Motivation: 深度伪造技术威胁信息真实性和安全性，现有检测模型复杂且资源消耗大，需更高效的解决方案。

Method: 使用轻量级CNN结合SE模块，动态调整通道特征，提升学习效率。
...
### [157] [Unsourced Adversarial CAPTCHA: A Bi-Phase Adversarial CAPTCHA Framework](https://arxiv.org/abs/2506.10685)
...
<details>
  <summary>Details</summary>
Motivation: 传统CAPTCHA方案在深度学习的攻击下日益脆弱，现有对抗攻击方法依赖原始图像特征，导致失真且适用性受限。

Method: 提出UAC框架，利用大型语言模型（LLM）生成对抗样本，支持定向和非定向攻击；针对非定向攻击，提出BP-UAC方法，采用多模态梯度和双路径优化策略。

Result: 实验表明，BP-UAC在多种系统中实现了高攻击成功率，生成的CAPTCHA对人类和DNN均难以区分。

Conclusion: UAC框架显著提升了CAPTCHA的多样性和对抗攻击的适用性，为安全领域提供了新思路。
...
### [158] [Underage Detection through a Multi-Task and MultiAge Approach for Screening Minors in Unconstrained Imagery](https://arxiv.org/abs/2506.10689)
*Christopher Gaul,Eduardo Fidalgo,Enrique Alegre,Rocío Alaiz Rodríguez,Eri Pérez Corral*

Main category: cs.CV

TL;DR: 论文提出了一种多任务架构，结合FaRL视觉-语言骨干网络和MLP，用于未成年人自动筛查，并通过改进的损失函数和采样策略解决数据不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 解决未成年人数据在公开数据中代表性不足及分布偏移问题，提升年龄估计和未成年人检测的准确性。

Method: 采用多任务架构，结合FaRL骨干网络和MLP，引入α加权焦点损失和年龄平衡采样策略，并定义严格评估基准。

Result: 在ASORES-39k测试集上，年龄估计误差降低，未成年人检测F2分数提升；在ASWIFT-20k测试集上表现出强泛化能力。

Conclusion: 提出的方法在年龄估计和未成年人检测任务中表现优异，尤其在分布偏移情况下具有强鲁棒性。
...
### [159] [Continual Hyperbolic Learning of Instances and Classes](https://arxiv.org/abs/2506.10710)
*Melika Ayoughi,Mina Ghadimi Atigh,Mohammad Mahdi Derakhshani,Cees G. M. Snoek,Pascal Mettes,Paul Groth*

Main category: cs.CV

TL;DR: 论文提出了一种名为HyperCLIC的持续学习算法，用于同时处理实例和类别的持续学习任务，利用双曲空间建模层次关系。
...
### [160] [Uncertainty-Masked Bernoulli Diffusion for Camouflaged Object Detection Refinement](https://arxiv.org/abs/2506.10712)
*Yuqi Shen,Fengyang Xiao,Sujie Hu,Youwei Pang,Yifan Pu,Chengyu Fang,Xiu Li,Chunming He*

Main category: cs.CV

TL;DR: 论文提出了一种名为UMBD的生成式细化框架，用于伪装目标检测（COD），通过不确定性引导的掩蔽机制和伯努利扩散，选择性优化分割质量较差的区域。


<details>
  <summary>Details</summary>
Motivation: 现有COD方法在目标与背景视觉差异微小的情况下表现有限，尤其是后处理细化方面尚未充分探索。

Method: UMBD结合不确定性引导的掩蔽机制和伯努利扩散，选择性优化残差区域；HUQNet通过多分支架构融合多源不确定性，提升估计精度。

Result: 在多个COD基准测试中，UMBD平均提升5.5%的MAE和3.2%的加权F-measure，计算开销较低。

Conclusion: UMBD是一种高效且通用的COD后处理细化框架，可无缝集成现有模型，显著提升性能。
...
### [161] [Deep Learning-based Multi Project InP Wafer Simulation for Unsupervised Surface Defect Detection](https://arxiv.org/abs/2506.10713)
...
TL;DR: 论文提出了一种基于深度神经网络的方法，通过从CAD数据生成合成的金标准图像，解决了InP晶圆制造中因缺乏金标准而依赖人工缺陷检测的问题。


<details>
  <summary>Details</summary>
Motivation: InP晶圆制造中，由于生产规模小和设计变异性高，通常缺乏金标准图像，导致缺陷检测依赖人工且效率低下。

Method: 使用深度神经网络从CAD数据生成逼真的InP晶圆图像，评估不同训练目标，并与基于决策树的基线方法进行比较。

Result: 深度学习方法优于基线方法，能够从CAD计划生成模拟金标准图像，用于更高效的缺陷检测。
...
### [162] [IQE-CLIP: Instance-aware Query Embedding for Zero-/Few-shot Anomaly Detection in Medical Domain](https://arxiv.org/abs/2506.10730)
...
<details>
  <summary>Details</summary>
Motivation: 现有CLIP方法依赖特定场景的提示词，难以区分正常和异常实例，且医学领域研究较少。

Method: 引入基于类和可学习的提示词，设计实例感知查询模块，提取多模态区域级信息。

Result: 在六个医学数据集上，IQE-CLIP在零样本和少样本设置中达到最先进性能。
...
### [163] [PosterCraft: Rethinking High-Quality Aesthetic Poster Generation in a Unified Framework](https://arxiv.org/abs/2506.10741)
...
<details>
  <summary>Details</summary>
Motivation: 生成具有艺术性和视觉吸引力的海报比简单设计更具挑战性，需要文本渲染与艺术内容的无缝融合。

Method: PosterCraft采用级联工作流，包括大规模文本渲染优化、区域感知微调、美学文本强化学习和联合视觉语言反馈细化。

Result: PosterCraft在渲染准确性、布局一致性和视觉吸引力上显著优于开源基线，接近商业系统的质量。
...
### [164] [Stroke-based Cyclic Amplifier: Image Super-Resolution at Arbitrary Ultra-Large Scales](https://arxiv.org/abs/2506.10774)
*Wenhao Guo,Peng Lu,Xujun Peng,Zhaoran Zhao,Sheng Li*

Main category: cs.CV

TL;DR: 提出了一种名为SbCA的统一模型，用于解决超大规模上采样任务中的性能下降问题，通过基于笔画的向量放大器和细节补全模块实现高保真图像重建。


<details>
  <summary>Details</summary>
Motivation: 现有ASISR方法在超出训练数据范围的上采样因子下性能显著下降，导致模糊问题。

Method: 使用基于笔画的向量放大器分解图像为向量图形进行放大，并通过细节补全模块恢复缺失细节，采用循环策略迭代优化。

Result: 在合成和真实数据集上验证，SbCA在超大规模上采样任务（如×100）中显著优于现有方法，生成更高质量的图像。

Conclusion: SbCA有效解决了分布漂移问题，消除了伪影、噪声和模糊，为超大规模上采样提供了高质量解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 人类通过观察世界获取物理知识并应用于动态推理，但现有方法忽视了物理知识的集成和模型在多样化场景中的适应性验证。
...
### [166] [Human-Robot Navigation using Event-based Cameras and Reinforcement Learning](https://arxiv.org/abs/2506.10790)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于图像的控制器存在固定速率、运动模糊和延迟问题，而事件相机的异步特性可解决这些问题。
...
### [167] [Prompts to Summaries: Zero-Shot Language-Guided Video Summarization](https://arxiv.org/abs/2506.10807)
...
Method: 分四步：(i)视频分段，(ii)批量生成场景描述，(iii)用LLM评分，(iv)通过一致性和独特性指标传播分数。

Result: 在SumMe和TVSum上超越无监督方法，在QFVS上与监督方法竞争，并发布新数据集VidSum-Reason。
...
### [168] [Unsupervised Deformable Image Registration with Structural Nonparametric Smoothing](https://arxiv.org/abs/2506.10813)
*Hang Zhang,Xiang Chen,Renjiu Hu,Rongguang Wang,Jinwei Zhang,Min Liu,Yaonan Wang,Gaolei Li,Xinxing Cheng,Jinming Duan*

Main category: cs.CV

TL;DR: SmoothProper是一种即插即用的神经模块，通过引入对偶优化层和定制交互项，解决了稀疏特征和大位移问题，提升了无监督变形图像配准的精度。


<details>
  <summary>Details</summary>
Motivation: 传统无监督变形图像配准方法在处理稀疏特征和大位移问题时表现不佳，因为网络预测的变形场在训练后不受约束，且正则化负担完全由网络权重承担。

Method: 提出SmoothProper模块，集成对偶优化层和定制交互项，实现流信号的空间传播、平滑性约束和结构一致性保持。

Result: 在视网膜血管数据集上，SmoothProper将配准误差降至1.88像素（2912x2912图像），首次有效解决了稀疏特征和大位移问题。

Conclusion: SmoothPrope是一种模型无关的解决方案，无需调整正则化超参数，可无缝集成到现有配准框架中，显著提升了无监督配准的性能。
...
### [169] [Occlusion-Aware 3D Hand-Object Pose Estimation with Masked AutoEncoders](https://arxiv.org/abs/2506.10816)
*Hui Yang,Wei Sun,Jian Liu,Jin Zheng,Jian Xiao,Ajmal Mian*

Main category: cs.CV

TL;DR: 论文提出了一种基于掩码自编码器的遮挡感知手-物体姿态估计方法（HOMAE），通过目标聚焦掩码策略和多尺度特征融合，提升了遮挡情况下的性能。
...
Method: 提出目标聚焦掩码策略，结合多尺度特征预测带符号距离场（SDF），并与点云融合，增强几何感知。

Result: 在DexYCB和HO3Dv2基准测试中达到最先进性能。

Conclusion: HOMAE通过结合全局上下文和局部几何信息，显著提升了遮挡情况下的手-物体姿态估计效果。
...
### [170] [VideoDeepResearch: Long Video Understanding With Agentic Tool Using](https://arxiv.org/abs/2506.10821)
*Huaying Yuan,Zheng Liu,Junjie Zhou,Ji-Rong Wen,Zhicheng Dou*

Main category: cs.CV

TL;DR: VideoDeepResearch提出了一种基于文本推理模型和多模态工具包的新型框架，无需依赖多模态大语言模型即可显著提升长视频理解任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型（MLLMs）在长视频理解（LVU）任务中存在复杂性和上下文窗口限制的问题，传统方法认为需要扩展上下文窗口和增强视觉感知能力。本文挑战了这一观点。

Method: 通过结合文本推理模型（LRM）和模块化多模态工具包（包括多模态检索器和视觉感知器），系统通过推理制定问题解决策略，并选择性利用视频内容。

Result: 在MLVU、Video-MME和LVBench等基准测试中，VideoDeepResearch分别比现有MLLM基线提升了9.6%、6.6%和3.9%。

Conclusion: 研究表明，代理系统在解决LVU任务中的关键挑战方面具有潜力，无需依赖复杂的多模态大语言模型。
...
### [171] [Post-Training Quantization for Video Matting](https://arxiv.org/abs/2506.10840)
*Tianrui Zhu,Houyuan Chen,Ruihao Gong,Michele Magno,Haotong Qin,Kai Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种专为视频抠图模型设计的后训练量化（PTQ）框架PTQ4VM，通过两阶段策略、全局仿射校准和光流辅助组件，显著提升了量化模型的精度和时序一致性，在低比特量化下接近全精度性能。


<details>
  <summary>Details</summary>
Motivation: 视频抠图在资源受限设备上部署时面临计算密集型模型的挑战，现有的后训练量化方法在精度和时序一致性上存在不足。

Method: 提出两阶段PTQ策略（块重建优化和全局校准）、统计驱动的全局仿射校准（GAC）方法，以及光流辅助（OFA）组件。

Result: PTQ4VM在不同比特宽度下均达到最先进精度，4比特量化下性能接近全精度模型，计算量减少8倍。
...
### [172] [VRBench: A Benchmark for Multi-Step Reasoning in Long Narrative Videos](https://arxiv.org/abs/2506.10857)
...
Method: 采用多阶段过滤和专家评审筛选视频，设计多步问答对和推理步骤，提出多维度评分指标。

Result: 评估了12个LLM和16个VLM，提供了多步推理领域的深入分析。

Conclusion: VRBench为多步推理研究提供了重要工具和见解。
...
### [173] [CreatiPoster: Towards Editable and Controllable Multi-Layer Graphic Design Generation](https://arxiv.org/abs/2506.10890)
*Zhao Zhang,Yutao Cheng,Dexiang Hong,Maoke Yang,Gonglei Shi,Lei Ma,Hui Zhang,Jie Shao,Xinglong Wu*

Main category: cs.CV

TL;DR: CreatiPoster是一个生成可编辑、多层图形设计的框架，通过自然语言指令或素材生成专业级设计，超越现有开源和商业工具。


<details>
  <summary>Details</summary>
Motivation: 当前AI工具在图形设计中难以准确整合用户素材、保持可编辑性并达到专业视觉效果，商业系统依赖模板库，难以复制。

Method: CreatiPoster使用协议模型生成JSON规范，详细描述每层布局、内容和样式，再通过条件背景模型合成背景。

Result: CreatiPoster在图形设计生成基准测试中表现优于开源和商业系统，并发布了10万份无版权多层设计数据集。
...
### [174] [AIR: Zero-shot Generative Model Adaptation with Iterative Refinement](https://arxiv.org/abs/2506.10895)
*Guimeng Liu,Milad Abdollahzadeh,Ngai-Man Cheung*

Main category: cs.CV

TL;DR: 论文提出了一种零样本生成模型适应方法（ZSGM），通过分析CLIP嵌入空间中文本偏移与图像偏移的不对齐问题，提出了一种迭代优化方法（AIR），显著提升了生成图像质量。
...
Method: 通过实证研究分析CLIP嵌入空间中文本与图像偏移的不对齐现象，发现概念距离与偏移不对齐相关。提出迭代优化方法（AIR）以提升目标域图像质量。

Result: 在26种实验设置中，AIR方法在定性和定量评估以及用户研究中均达到最优性能。

Conclusion: AIR方法通过关注偏移不对齐问题，显著提升了零样本生成模型适应性能，为未来研究提供了新方向。
...
### [175] [M4V: Multi-Modal Mamba for Text-to-Video Generation](https://arxiv.org/abs/2506.10915)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的文本到视频生成方法计算复杂度高，限制了实际应用。Mamba架构虽高效，但其简单设计难以直接适用于多模态和时空视频生成任务。

Method: 提出M4V框架，采用MM-DiM块实现多模态信息融合和时空建模，并通过奖励学习策略提升长上下文生成中的视觉质量。

Result: M4V在768×1280分辨率视频生成中，FLOPs减少45%，并在文本到视频基准测试中表现出高质量生成能力。

Conclusion: M4V通过高效的多模态Mamba框架，显著降低了计算成本并提升了视频生成质量，具有实际应用潜力。
...
### [176] [VINCIE: Unlocking In-context Image Editing from Video](https://arxiv.org/abs/2506.10941)
*Leigang Qu,Feng Cheng,Ziyan Yang,Qi Zhao,Shanchuan Lin,Yichun Shi,Yicong Li,Wenjie Wang,Tat-Seng Chua,Lu Jiang*

Main category: cs.CV

TL;DR: 论文提出了一种直接从视频中学习上下文图像编辑模型的方法，通过标注视频为多模态序列，并设计块因果扩散变换器进行训练，实现了多任务学习和多轮图像编辑的先进性能。
...
Method: 提出了一种可扩展的视频标注方法，将其转化为多模态序列，并设计块因果扩散变换器，训练于三个代理任务：下一图像预测、当前分割预测和下一分割预测。

Result: 模型在上下文图像编辑任务中表现出色，并在多轮图像编辑基准测试中达到最先进水平，同时展现出多概念组合、故事生成和编辑链应用的潜力。

Conclusion: 直接从视频中学习上下文图像编辑模型是可行的，且能实现多功能应用，为研究提供了新方向。
...
TL;DR: 提出了一种基于频谱视角的自回归视觉生成框架SpectralAR，通过有序频谱令牌实现视觉序列的因果性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将图像块作为并行序列处理，与自回归建模的因果性相矛盾。

Method: 使用嵌套频谱令牌化将图像转换为有序频谱令牌，并按从低频到高频的顺序进行自回归生成。

Result: 在ImageNet-1K上，SpectralAR仅用64个令牌和310M参数实现了3.02 gFID。

Conclusion: SpectralAR在保持序列因果性和令牌效率的同时，实现了高质量的视觉生成。
...
TL;DR: 论文提出了知识图像生成任务和MMMG基准，用于评估图像生成模型的推理能力，发现现有模型在推理上存在严重缺陷，并提出了一个开放基线FLUX-Reason。
...
Method: 提出了MMMG基准，包含4,456个专家验证的知识图像-提示对，采用统一的知识图谱表示，并引入MMMG-Score评估生成图像的质量。

Result: 评估16个先进文本到图像生成模型，发现推理能力不足，GPT-4o的MMMG-Score仅为50.20。
...
TL;DR: CDPruner是一种新型视觉令牌剪枝方法，通过最大化条件多样性优化多模态大语言模型（MLLMs）的推理成本。


<details>
  <summary>Details</summary>
Motivation: 解决MLLMs中视觉令牌冗余导致的推理成本高问题，现有方法存在重复令牌或忽略指令相关性的缺陷。

Method: 提出基于条件多样性的剪枝方法CDPruner，利用条件相似性和DPP最大化保留令牌的多样性。

Result: 在多种MLLMs上实现SOTA性能，显著降低FLOPs和延迟，同时保持高准确率。

Conclusion: CDPruner通过条件多样性优化剪枝，高效平衡性能和计算成本，适用于多种MLLMs。
...
TL;DR: 本文提出了GenWorld，一个用于AI生成视频检测的大规模、高质量、真实世界模拟数据集，并开发了SpannDetector模型，利用多视角一致性检测AI生成视频。


<details>
  <summary>Details</summary>
Motivation: 视频生成技术的发展威胁了真实世界信息的可信度，亟需高质量的AI生成视频检测工具，但现有数据集质量不足。

Method: 构建GenWorld数据集，模拟真实世界场景，结合多种生成模型和提示模态；提出SpannDetector模型，利用多视角一致性检测AI视频。

Result: 实验表明SpannDetector在检测高质量AI生成视频上表现优异，揭示了基于物理合理性的可解释检测方向。

Conclusion: GenWorld数据集和SpannDetector模型为AI生成视频检测领域提供了重要贡献。
...
TL;DR: 论文提出了一种基于超二次曲面的3D占用预测方法QuadricFormer，通过几何表达力强的超二次曲面作为场景基元，提高了建模效率，并在nuScenes数据集上实现了最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在3D占用预测中通常使用密集体素或稀疏高斯表示，前者效率低，后者因椭球形先验限制无法准确建模多样结构。

Method: 提出使用超二次曲面作为场景基元，开发概率超二次曲面混合模型，并设计QuadricFormer模型及剪枝-分裂模块以优化效率。

Result: 在nuScenes数据集上，QuadricFormer在保持高效的同时实现了最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有注意力扰动方法缺乏确定扰动应用位置的原则性方法，特别是在DiT架构中，质量相关计算分布在多个层中。

Method: 研究了注意力扰动的粒度（从层到单个注意力头），提出HeadHunter框架和SoftPAG方法，通过线性插值调节扰动强度。

Result: 在Stable Diffusion 3和FLUX.1等模型上验证了方法的优越性，实现了生成质量提升和风格特定引导。

Conclusion: 首次在扩散模型中进行了头级注意力扰动分析，揭示了注意力层的可解释性专业化，为设计有效扰动策略提供了实践指导。
...
TL;DR: InstaInpaint是一个基于参考的前馈框架，能在0.4秒内完成3D场景修复，速度提升1000倍，同时保持高性能。
...
TL;DR: SceneCompleter是一个通过密集3D场景补全实现3D一致生成新视角合成的框架，解决了现有方法在2D补全和3D恢复中导致的平滑表面和几何扭曲问题。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型在2D补全后通过3D恢复技术重建场景，导致表面过于平滑和几何扭曲，难以仅从RGB数据推断3D结构。

Method: 提出SceneCompleter框架，包含几何-外观双流扩散模型（在RGBD空间联合合成新视角）和场景编码器（从参考图像编码全局场景理解）。

Result: 方法在生成新视角合成中表现出更高的连贯性和合理性，优于现有方法。

Conclusion: SceneCompleter通过融合结构和纹理信息，实现了视觉连贯和3D一致的场景补全。
...
### [185] [HPCTransCompile: An AI Compiler Generated Dataset for High-Performance CUDA Transpilation and LLM Preliminary Exploration](https://arxiv.org/abs/2506.10401)
...
Method: 提出了一种结合AI编译器和自动优化技术的框架，采用图数据增强方法，并引入HPCTransEval基准测试。

Result: 实验表明，该框架显著提升了CUDA转译性能，展示了LLM解决CUDA生态系统兼容性问题的潜力。

Conclusion: 该研究为高性能代码转译提供了新思路，并验证了LLM在此领域的应用前景。
...
### [186] [FASCIST-O-METER: Classifier for Neo-fascist Discourse Online](https://arxiv.org/abs/2506.10789)
*Rudy Alexandro Garrido Veliz,Martin Semmann,Chris Biemann,Seid Muhie Yimam*

Main category: cs.CY

TL;DR: 该研究开发了首个针对美国数字话语的新法西斯主义编码方案，结合NLP与政治学，通过标注论坛帖子训练分类模型，发现新法西斯言论普遍存在，强调社会背景的重要性。


<details>
  <summary>Details</summary>
Motivation: 新法西斯主义在西方社会迅速增长，对民主和少数群体构成威胁，需采取行动遏制其蔓延。

Method: 收集新法西斯主义论坛数据，通过众包标注帖子，训练和测试SLMs和LLMs分类模型。

Result: 研究发现新法西斯言论在论坛中普遍存在，分类模型成功识别此类内容。

Conclusion: 研究强调持续对抗新法西斯主义的重要性，并指出社会背景在NLP研究中的关键作用。
...
### [187] [LLM-Driven Personalized Answer Generation and Evaluation](https://arxiv.org/abs/2506.10829)
...
<details>
  <summary>Details</summary>
Motivation: 在线学习的灵活性和可访问性推动了其快速发展，个性化学习体验是关键，尤其是通过定制化答案满足学习者需求。

Method: 研究开发了验证框架和数据集，采用0-shot、1-shot和few-shot策略生成个性化答案，并通过BERTScore、LLM评估和人工评估三种方法验证效果。

Result: 结果表明，为LLMs提供学习者或类似学习者的示例答案能显著提升其个性化回答能力。

Conclusion: LLMs在生成个性化学习答案方面具有潜力，能增强学习参与度并减轻教育者负担。
...
### [188] [AniMaker: Automated Multi-Agent Animated Storytelling with MCTS-Driven Clip Generation](https://arxiv.org/abs/2506.10540)
*Haoyuan Shi,Yunxin Li,Xinyu Chen,Longyue Wang,Baotian Hu,Min Zhang*

Main category: cs.MA

TL;DR: AniMaker是一个多智能体框架，通过高效的候选片段生成和故事感知的片段选择，从文本输入生成全局一致且故事连贯的动画。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法在多场景和角色转换时存在叙事不连贯和节奏问题，且生成质量不稳定，影响整体动画的逻辑和视觉一致性。

Method: AniMaker采用多智能体框架，包括导演、摄影、评审和后期制作智能体，结合MCTS-Gen和AniEval技术，优化片段生成和评估。

Result: 实验表明，AniMaker在VBench和AniEval等指标上表现优异，显著提升了多候选生成的效率。

Conclusion: AniMaker推动了AI生成的故事动画向生产标准迈进。
...
### [189] [Omni-DPO: A Dual-Perspective Paradigm for Dynamic Preference Learning of LLMs](https://arxiv.org/abs/2506.10054)
...
### [190] [Textual Bayes: Quantifying Uncertainty in LLM-Based Systems](https://arxiv.org/abs/2506.10060)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在不确定性量化方面存在挑战，且其闭源性和黑盒特性加剧了这一问题。此外，LLM系统对提示的敏感性需要大量手动调整。

Method: 将提示视为文本参数，利用小规模训练数据进行贝叶斯推断，并提出了MHLP算法，结合提示优化技术和标准MCMC方法。

Result: 实验表明，该方法在预测准确性和不确定性量化方面均有提升。

Conclusion: 该研究为将贝叶斯方法融入LLM时代提供了可行路径，有助于构建更可靠和校准的LLM系统。
...
### [191] [Learning to Collaborate Over Graphs: A Selective Federated Multi-Task Learning Approach](https://arxiv.org/abs/2506.10102)
*Ahmed Elbakary,Chaouki Ben Issaid,Mehdi Bennis*

Main category: cs.LG

TL;DR: 提出了一种新颖的联邦多任务学习方法，通过跨客户端相似性实现个性化学习，并采用通信高效的特征锚和动态图协作机制。


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中个性化学习的需求，同时减少通信开销和避免负协作。

Method: 引入特征锚和分类头共享，结合动态图协作和社区检测机制，限制协作范围。

Result: 在异构数据集上显著优于现有方法，同时提升了计算和通信效率及公平性。

Conclusion: 该方法有效平衡了个性化与协作，具有高效性和公平性。
...
### [192] [GRAIL: A Benchmark for GRaph ActIve Learning in Dynamic Sensing Environments](https://arxiv.org/abs/2506.10120)
*Maryam Khalid,Akane Sano*

Main category: cs.LG

TL;DR: GRAIL是一个新的基准框架，用于评估动态环境中基于图的主动学习（AL）策略，弥补了现有方法在多样性、公平性和动态适应性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的AL方法主要在静态数据集上评估，忽视了用户中心因素（如多样性、公平性）和动态环境适应性。

Method: 引入GRAIL框架，提出新指标评估AL方法的持续性、多样性和用户负担。

Result: 实验显示现有AL方法在动态环境中存在预测性能和用户负担之间的权衡问题。
...
### [193] [Self-Predictive Representations for Combinatorial Generalization in Behavioral Cloning](https://arxiv.org/abs/2506.10137)
...
Conclusion: BYOL-γ增强的GCBC方法通过改进时间一致性，显著提升了组合泛化能力，为行为克隆提供了新思路。
...
### [194] [Interpreting learned search: finding a transition model and value function in an RNN that plays Sokoban](https://arxiv.org/abs/2506.10138)
*Mohammad Taufeeque,Aaron David Tucker,Adam Gleave,Adrià Garriga-Alonso*

Main category: cs.LG

TL;DR: 该论文通过逆向工程分析了一个用于玩Sokoban游戏的卷积循环神经网络（RNN），揭示了其类似于经典双向搜索的机制，并探讨了其与经典搜索的不同之处。


<details>
  <summary>Details</summary>
Motivation: 研究目的是理解模型自由强化学习训练的RNN如何利用测试时计算解决Sokoban游戏，并揭示其内部机制。

Method: 通过部分逆向工程分析RNN的激活通道和方向关联，发现其类似于双向搜索的机制，包括状态-动作激活和路径扩展。

Result: 研究发现RNN通过方向关联的通道表示计划，激活幅度决定回溯和修剪，且网络具有分层计划和价值函数。

Conclusion: 该网络的机制虽不同于经典搜索，但可通过熟悉的概念理解，表明模型自由训练的RNN具有可解释性。
...
### [195] [A Comparative Study of Machine Learning Techniques for Early Prediction of Diabetes](https://arxiv.org/abs/2506.10180)
...
Method: 使用Pima Indians Diabetes数据集，比较了Logistic回归、决策树、随机森林、k近邻、朴素贝叶斯、支持向量机、梯度提升和神经网络等算法的表现。

Result: 神经网络算法表现最佳，准确率为78.57%，随机森林次之，准确率为76.30%。

Conclusion: 机器学习算法可用于糖尿病预测，是一种有效的早期检测工具。
...
### [196] [Optimizing Genetic Algorithms with Multilayer Perceptron Networks for Enhancing TinyFace Recognition](https://arxiv.org/abs/2506.10184)
*Mohammad Subhi Al-Batah,Mowafaq Salem Alzboon,Muhyeeddin Alqaraleh*

Main category: cs.LG

TL;DR: 该研究通过三种不同数据集（TinyFace、Heart Disease和Iris）对MLP网络进行了实证分析，比较了基线训练、遗传算法特征选择和PCA降维三种方法的效果。结果表明，PCA在低维无噪声数据中表现良好，而GA在复杂数据中能显著提升准确性。
...
Method: 采用三种方法：1) 默认设置的MLP基线训练；2) 基于遗传算法的特征选择；3) PCA降维。

Result: PCA适用于低维无噪声数据，GA在复杂数据中能更准确地识别关键特征并提升性能。

Conclusion: 特征选择和降维技术对MLP性能有显著影响，研究为特征工程和神经网络优化提供了实用指南。
...
### [197] [Scalable Non-Equivariant 3D Molecule Generation via Rotational Alignment](https://arxiv.org/abs/2506.10186)
...
Result: 性能显著优于非等变模型，质量与最先进等变模型相当，训练和采样效率更高。

Conclusion: 放松等变性约束的方法有效，兼具高质量和高效率。
...
### [198] [Cross-Learning Between ECG and PCG: Exploring Common and Exclusive Characteristics of Bimodal Electromechanical Cardiac Waveforms](https://arxiv.org/abs/2506.10212)
...
<details>
  <summary>Details</summary>
Motivation: ECG和PCG分别捕捉心脏的电活动和机械活动，但两者信号间的重叠和独特信息及其相互重建潜力尚未完全理解，尤其是在不同生理条件和个体间。

Method: 使用EPHNOGRAM数据集，结合线性和非线性机器学习模型（如非因果LSTM），分析ECG和PCG的相互重建能力，并探讨因果关系、生理状态和跨受试者变异性的影响。

Result: 非线性模型（尤其是非因果LSTM）在重建中表现最佳，ECG从PCG重建比反向更容易。运动场景和跨受试者情况更具挑战性，但基于包络的建模显著提高了跨模态学习的泛化能力。
...
### [199] [LaMAGIC2: Advanced Circuit Formulations for Language Model-Based Analog Topology Generation](https://arxiv.org/abs/2506.10235)
*Chen-Chia Chang,Wan-Hsuan Lin,Yikang Shen,Yiran Chen,Xin Zhang*

Main category: cs.LG

TL;DR: LaMAGIC2提出了一种新的模拟拓扑生成方法SFCI，解决了现有方法在电路表示效率和数值精度上的不足，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现代应用对模拟拓扑设计的定制化需求高，但现有方法在电路表示和数值精度上存在不足，亟需改进。

Method: 采用SFCI（简洁浮点输入规范表示）方法，通过基于标识符的表示优化组件类型识别，降低复杂度至O(|V|)，并提升数值精度敏感性。
...
TL;DR: 论文提出了一种基于元学习的恶意傀儡账户检测方法，显著提高了预测精度，并在数据稀缺情况下表现出色。


<details>
  <summary>Details</summary>
Motivation: 维基百科上的恶意傀儡账户检测对维护互联网可靠信息和防止虚假信息传播至关重要。现有方法未能有效适应特定作者行为，尤其在数据有限时表现不佳。
...
TL;DR: 本文提出了从语言反馈中学习（LLF）问题的形式化框架，引入了转移逃避维度作为衡量问题复杂性的指标，并开发了一种无遗憾算法HELiX，证明了其在LLF问题中的有效性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型（LLM）代理的出现，从观察和语言反馈中交互式学习成为一个热门研究领域，但目前缺乏对这些决策问题的理论框架。

Method: 本文形式化了LLF问题，提出了转移逃避维度作为复杂性度量，并开发了HELiX算法，通过顺序交互解决LLF问题。

Result: 研究表明，转移逃避维度能够捕捉反馈信息对学习复杂性的影响，且在某些情况下，从丰富语言反馈中学习比从奖励中学习快指数级。HELiX算法在多个实证领域表现优异。

Conclusion: 本文为从通用语言反馈中设计有原则的交互式学习算法迈出了第一步，展示了HELiX算法的理论保证和实证效果。
...
TL;DR: 本文研究了针对大型语言模型（LLMs）的属性推断攻击，提出了PropInfer基准任务，并设计了两种攻击方法，揭示了LLMs在微调过程中的潜在漏洞。
...
Result: 实验表明，这两种攻击方法在多种预训练LLMs上均取得成功，揭示了LLMs在微调过程中的新漏洞。

Conclusion: LLMs在微调过程中存在属性推断攻击的潜在风险，需要进一步研究防御措施。
...
TL;DR: 论文提出了一种因果表示学习框架，通过建模基准表现为潜在能力因素的线性变换，揭示了模型能力间的因果关系。
...
Method: 采用因果表示学习框架，将基准表现建模为潜在能力因素的线性变换，并通过控制基础模型作为混杂变量来识别因果关系。

Result: 在包含1500多个模型的数据集中，识别出一个三节点线性因果结构，揭示了从通用问题解决到数学推理的因果方向。

Conclusion: 研究强调了控制基础模型变体对准确揭示模型能力间因果关系的重要性。
...
TL;DR: Time-IMM数据集和IMM-TSF基准库旨在解决现实世界中不规则、多模态时间序列数据的挑战，提供了专门的数据集和工具以支持异步整合和真实评估。
...
Method: 提出Time-IMM数据集，捕捉九种不规则性机制；开发IMM-TSF基准库，包含时间戳到文本和多模态融合模块。
...
TL;DR: 提出了一种两阶段方法，用于准确数字化纸基心电图（ECG）记录，特别针对信号重叠问题。通过U-Net分割和自适应网格检测，显著提高了数字化精度。
...
Method: 两阶段流程：1）U-Net分割网络隔离ECG主迹线；2）自适应网格检测将二值掩码转换为时间序列信号。

Result: U-Net分割IoU达0.87；数字化方法在非重叠和重叠信号上均优于基线（MSE和Pearson系数显著提升）。

Conclusion: 该方法显著提高了ECG数字化精度，尤其在信号重叠情况下，为研究和临床提供了可靠工具。
...
TL;DR: 论文提出了一种自动化问答方法Neural，用于电子健康记录（EHR）的临床问答任务，通过分阶段证据检索和答案生成，结合提示优化和自一致性投票，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 解决临床问答中信息检索和答案生成的精确性问题，同时减少对监督数据的依赖。

Method: 将任务分为句子级证据识别和带引用的答案生成两阶段，利用DSPy的MIPROv2优化器自动探索提示空间，并结合自一致性投票提高证据召回率。
...
### [207] [PhysioWave: A Multi-Scale Wavelet-Transformer for Physiological Signal Representation](https://arxiv.org/abs/2506.10351)
...
Method: 采用小波变换捕捉多尺度时频特征，并首次针对EMG和ECG信号训练大规模预训练模型；构建多模态框架，融合EEG模型。

Result: 在多项下游任务中表现优异，解决了低信噪比、高个体差异和设备不匹配等问题，性能超越现有方法。

Conclusion: 该方法为生理信号分析提供了新思路，多模态设计在健康监测和临床诊断等领域具有广泛应用潜力。
...
### [208] [The Diffusion Duality](https://arxiv.org/abs/2506.10892)
*Subham Sekhar Sahoo,Justin Deschenaux,Aaron Gokaslan,Guanghan Wang,Justin Chiu,Volodymyr Kuleshov*

Main category: cs.LG

TL;DR: 论文提出了一种名为Duo的方法，通过将高斯扩散技术引入均匀状态扩散模型，提升了文本生成的性能。该方法通过课程学习和离散一致性蒸馏技术，显著提高了训练速度和采样效率。


<details>
  <summary>Details</summary>
Motivation: 均匀状态扩散模型虽然具有自我纠正能力，但在性能上通常落后于自回归模型和掩码扩散模型。论文旨在缩小这一性能差距。

Method: Duo方法利用高斯扩散的底层特性，引入课程学习策略和离散一致性蒸馏技术，优化训练和采样过程。

Result: 使用课程学习的模型在7个基准测试中的3个上超越了自回归模型的零样本困惑度；离散一致性蒸馏技术将采样速度提高了两个数量级。

Conclusion: Duo方法通过结合高斯扩散技术，显著提升了均匀状态扩散模型的性能，为快速文本生成提供了新的解决方案。
...
### [209] [Robustly Improving LLM Fairness in Realistic Settings via Interpretability](https://arxiv.org/abs/2506.10922)
*Adam Karvonen,Samuel Marks*

Main category: cs.LG

TL;DR: 研究发现，在招聘场景中引入现实背景后，简单的反偏见提示无法消除大语言模型（LLM）的偏见。通过内部偏见缓解方法，可以显著降低偏见。


<details>
  <summary>Details</summary>
Motivation: LLM在招聘等高风险应用中的偏见问题直接影响人们的职业和生计，需要更有效的缓解策略。

Method: 通过识别和中和模型激活中的敏感属性方向，采用内部偏见缓解方法（如仿射概念编辑）。

Result: 在引入现实背景后，所有测试模型均表现出显著偏见（如种族和性别偏见）。内部缓解方法将偏见降至极低水平（通常低于1%）。

Conclusion: 建议招聘场景中的LLM部署采用更现实的评估方法，并考虑内部缓解策略以实现公平结果。
...
### [210] [Time To Impeach LLM-as-a-Judge: Programs are the Future of Evaluation](https://arxiv.org/abs/2506.10403)
*Tzu-Heng Huang,Harit Vishwakarma,Frederic Sala*

Main category: cs.LG

TL;DR: PAJAMA提出了一种基于程序合成的方法替代直接使用LLM评估生成内容，解决了高成本、不可靠性、不灵活性和偏见问题。
...
Method: 通过LLM合成可执行的评判程序，本地运行，降低成本并提供可解释、可审计的逻辑。

Result: 程序评判一致性提升15.83%，偏见减少23.7%，在CHAT-HARD子集上表现优于LLM评判。
...
### [211] [GUARD: Guided Unlearning and Retention via Data Attribution for Large Language Models](https://arxiv.org/abs/2506.10946)
*Evelyn Ma,Duo Zhou,Peizhi Niu,Huiting Zhou,Huan Zhang,Olgica Milenkovic,S. Rasoul Etesami*

Main category: cs.LG

TL;DR: GUARD是一个针对大语言模型（LLM）的轻量级去学习框架，通过数据归因指标减少无意遗忘，提升模型保留有用信息的能力。


<details>
  <summary>Details</summary>
Motivation: 由于法规合规、版权保护和隐私问题，LLM的去学习变得重要，但现有方法在去学习高影响力数据时容易导致保留性能下降。

Method: GUARD引入了一种轻量级代理数据归因指标，设计了自适应非均匀去学习权重的目标函数。

Result: 实验表明，GUARD在TOFU基准测试中显著提升了保留性能，同时保持去学习效果，Truth Ratio提升高达194.92%。

Conclusion: GUARD通过数据归因优化去学习过程，有效平衡了遗忘与保留的需求。
...
### [212] [Build the web for agents, not agents for the web](https://arxiv.org/abs/2506.10953)
*Xing Han Lù,Gaurav Kamath,Marius Mosbach,Siva Reddy*

Main category: cs.LG

TL;DR: 论文提出了一种新的网络代理交互范式——Agentic Web Interface (AWI)，旨在优化代理能力而非适应人类界面设计，以解决现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型（LLMs）和多模态模型在开发网络代理时面临界面设计与代理能力不匹配的挑战，导致处理复杂网络输入时效率低下。

Method: 提出了Agentic Web Interface (AWI)概念，并制定了六项设计原则，强调安全性、效率和标准化。

Result: AWI旨在克服现有界面的根本限制，为更高效、可靠和透明的网络代理设计铺平道路。

Conclusion: 通过重新定义交互范式，AWI为网络代理的未来发展提供了新的方向，需要机器学习社区的广泛合作。
...
### [213] [Balanced Hyperbolic Embeddings Are Natural Out-of-Distribution Detectors](https://arxiv.org/abs/2506.10146)
*Tejaswi Kasarla,Max van Spengler,Pascal Mettes*

Main category: cs.LG

TL;DR: 该论文提出了一种基于双曲嵌入的方法（Balanced Hyperbolic Learning），用于区分分布内和分布外样本，并在实验中表现优于现有方法。
...
Method: 提出平衡双曲学习算法，优化层次失真和子层次平衡，并使用双曲原型进行分类和分布外评分。

Result: 在13个数据集和13种评分函数上的实验表明，该方法优于现有技术和对比学习方法。

Conclusion: 双曲嵌入在分布外识别中表现优越，并能自然支持层次化分布外泛化。
...
### [214] [Size-adaptive Hypothesis Testing for Fairness](https://arxiv.org/abs/2506.10586)
...
<details>
  <summary>Details</summary>
Motivation: 现有公平性评估方法依赖于单点估计与预设阈值的比较，忽略了抽样误差和不同规模子组的差异，尤其在交叉分析中数据稀疏性导致估计不可靠。

Method: （i）对大子组，证明了统计奇偶差异的中心极限定理，提供解析置信区间和Wald检验；（ii）对小子组，采用贝叶斯Dirichlet-多项式估计器，通过蒙特卡洛可信区间适应任意样本量。

Result: 在基准数据集上验证了方法的有效性，展示了在不同数据可用性和交叉性下的可解释、统计严谨的决策能力。

Conclusion: 该框架为公平性评估提供了统计严谨且适应数据规模的方法，解决了现有方法的局限性。
...
### [215] [Geometric Regularity in Deterministic Sampling of Diffusion-based Generative Models](https://arxiv.org/abs/2506.10177)
...
Result: 发现轨迹具有低维和“回旋镖”形状的规律性，动态规划方法显著提升了图像生成性能。

Conclusion: 轨迹规律性为优化采样提供了新思路，动态规划方法简单高效且性能优越。
...
### [216] [Task Adaptation from Skills: Information Geometry, Disentanglement, and New Objectives for Unsupervised Reinforcement Learning](https://arxiv.org/abs/2506.10629)
*Yucheng Yang,Tianyi Zhou,Qiang He,Lei Han,Mykola Pechenizkiy,Meng Fang*

Main category: cs.LG

TL;DR: 论文提出了一种新的理论分析，指出MISL方法在无监督强化学习中缺乏对技能多样性和可分离性的保证，并提出新的度量LSEPIN和基于Wasserstein距离的算法WSEP及PWSEP，以改进下游任务适应。


<details>
  <summary>Details</summary>
Motivation: MISL方法在无监督强化学习中缺乏对技能多样性和可分离性的理论分析，影响下游任务适应。

Method: 提出LSEPIN度量，基于Wasserstein距离设计新算法WSEP和PWSEP，优化技能学习目标。

Result: WSEP和PWSEP能发现更多初始策略，PWSEP理论上可发现所有最优初始策略。

Conclusion: 基于Wasserstein距离的方法在技能学习和下游任务适应中具有优势。
...
### [217] [Time Series Forecasting as Reasoning: A Slow-Thinking Approach with Reinforced LLMs](https://arxiv.org/abs/2506.10630)
...
### [218] [Data Shifts Hurt CoT: A Theoretical Study](https://arxiv.org/abs/2506.10647)
*Lang Yin,Debangshu Banerjee,Gagandeep Singh*

Main category: cs.LG

TL;DR: Chain of Thought (CoT) 提升大语言模型输出质量，但在数据分布变化和数据污染情况下，CoT 可能导致性能下降。本文研究 $k$-parity 问题中数据偏移对模型的影响。


<details>
  <summary>Details</summary>
Motivation: 现实世界中，训练和测试数据分布可能不一致，且数据可能被污染，现有研究未严格分析此类数据偏移对 CoT 模型的具体危害。

Method: 研究 $k$-parity 问题，分析数据分布偏移和数据污染的联合影响，对比 CoT 分解与直接预测的性能差异。

Result: 发现 CoT 在数据偏移情况下性能反而比直接预测更差，并给出机制性解释。

Conclusion: CoT 在理想条件下有效，但在数据偏移时可能适得其反，需进一步研究其鲁棒性。
...
### [219] [Saturation Self-Organizing Map](https://arxiv.org/abs/2506.10680)
...
<details>
  <summary>Details</summary>
Motivation: 持续学习中神经网络常面临灾难性遗忘问题，自组织映射（SOMs）虽高效但同样存在此问题。

Method: 提出SatSOM，引入饱和机制，逐步降低学习率和邻域半径，冻结训练充分的神经元并引导学习至未充分利用区域。

Result: SatSOM在持续学习场景中提高了知识保留能力。

Conclusion: SatSOM通过饱和机制有效改善了SOMs在持续学习中的表现。
...
### [220] [ConTextTab: A Semantics-Aware Tabular In-Context Learner](https://arxiv.org/abs/2506.10707)
...
<details>
  <summary>Details</summary>
Motivation: 现有表格原生ICL模型仅依赖合成数据，未能充分利用真实数据的语义信息；而基于大型语言模型的表格ICL模型受限于架构，只能利用少量上下文。
...
Method: 通过近似潜在变量的后验分布，学习对数配分函数，从而定义Fisher度量。提供理论收敛保证，并在Ising和TASEP模型上验证。

Result: 方法在重建热力学量上优于基线，揭示了扩散模型中相变的复杂结构，包括Fisher度量的突变和相边界处的非线性行为。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法如DPS在奖励信息不足时（如低信噪比的逆问题）容易偏离数据流形，导致输出不真实。

Method: 通过反转候选解并运行无条件概率流ODE，将结果作为DPS的初始化。

Result: 在困难逆问题（如大范围修复和高倍超分辨率）中，ReGuidance显著提升了样本质量和测量一致性。

Conclusion: ReGuidance首次为DPS提供了严格的算法保证，同时提升了奖励和数据流形接近度。
...
### [223] [Efficiency Robustness of Dynamic Deep Learning Systems](https://arxiv.org/abs/2506.10831)
...
<details>
  <summary>Details</summary>
Motivation: 动态深度学习系统通过动态调整计算提高效率，但同时也引入了新的攻击面，效率对抗攻击可能利用这些动态机制降低系统性能。

Method: 通过分类三种动态行为（动态计算、动态推理迭代、动态输出生成）的效率攻击，深入评估攻击策略。

Result: 研究发现现有防御机制对效率攻击的防护不足，需开发新的缓解策略。

Conclusion: 未来需要针对动态深度学习系统的效率攻击开发更有效的防御方法。
...
### [224] [Understanding In-Context Learning on Structured Manifolds: Bridging Attention to Kernel Methods](https://arxiv.org/abs/2506.10959)
*Zhaiming Shen,Alexander Hsu,Rongjie Lai,Wenjing Liao*

Main category: cs.LG

TL;DR: 本文通过理论分析研究了上下文学习（ICL）在流形上的Hölder函数回归问题，揭示了注意力机制与经典核方法的联系，并推导了泛化误差界。
...
Method: 建立注意力机制与核方法的新联系，分析变换器在流形上的回归性能。

Result: 当训练任务足够多时，变换器能达到流形上Hölder函数的最小最大回归率，误差随流形内禀维度指数增长。
...
### [225] [Farseer: A Refined Scaling Law in Large Language Models](https://arxiv.org/abs/2506.10972)
...
TL;DR: Farseer是一种新的扩展定律，通过改进的预测准确性帮助解决大规模语言模型训练的高成本问题。


<details>
  <summary>Details</summary>
Motivation: 解决小规模实验难以推广到高成本生产系统的问题，提高创新效率。

Method: 构建模型损失表面$L(N,D)$，提供比现有定律（如Chinchilla定律）更准确的预测。

Result: Farseer显著减少外推误差（433%），并提供对计算资源分配的更优见解。

Conclusion: Farseer为大规模语言模型训练提供了可靠预测工具，并开源所有资源以促进研究。
...
### [226] [Principled Approaches for Extending Neural Architectures to Function Spaces for Operator Learning](https://arxiv.org/abs/2506.10973)
...
TL;DR: 该论文提出了一种将神经网络推广到函数空间映射的方法，称为神经算子，旨在解决科学问题中的无限维函数空间映射问题。


<details>
  <summary>Details</summary>
Motivation: 科学问题（如连续时间动力系统和偏微分方程）通常涉及无限维函数空间，而深度学习主要应用于有限维空间，限制了其在科学领域的成功。神经算子提供了一种推广神经网络的方法。

Method: 论文提炼了构建无限维函数空间映射的关键原则，并提出了一种将现有神经网络架构转换为神经算子的方法，只需最小修改。

Result: 通过将流行的神经网络架构转换为神经算子，论文展示了神经算子在实践中的可行性，并提供了实现细节。

Conclusion: 神经算子为科学问题中的函数空间映射提供了一种实用方法，能够继承深度学习的经验优化，推动科学领域的深度学习应用。
...
### [227] [Rethinking Losses for Diffusion Bridge Samplers](https://arxiv.org/abs/2506.10982)
*Sebastian Sanokowski,Lukas Gruber,Christoph Bartmann,Sepp Hochreiter,Sebastian Lehner*

Main category: cs.LG

TL;DR: 论文分析了扩散桥方法中Log Variance (LV)损失和反向Kullback-Leibler (rKL)损失的性能差异，发现rKL损失结合对数导数技巧（rKL-LD）在性能和稳定性上优于LV损失。
...
### [228] [AC/DC: LLM-based Audio Comprehension via Dialogue Continuation](https://arxiv.org/abs/2506.10312)
*Yusuke Fujita,Tomoya Mizumoto,Atsushi Kojima,Lianbo Liu,Yui Sudo*

Main category: eess.AS

TL;DR: 提出了一种基于对话延续训练的音频理解模型，利用大语言模型的对话能力，解决字幕变异性问题，实现零样本指令跟随。


<details>
  <summary>Details</summary>
Motivation: 传统音频字幕生成方法存在字幕变异性问题，难以捕捉字幕的深层含义。

Method: 通过训练模型以对话延续方式生成响应，而非直接生成目标字幕，从而更好地理解字幕含义。
...
### [229] [RT-VC: Real-Time Zero-Shot Voice Conversion with Speech Articulatory Coding](https://arxiv.org/abs/2506.10289)
...
Method: 利用发音特征空间分离内容和说话人特征，结合DDSP技术直接从发音特征进行高效声码化。
...
### [230] [Robust Unsupervised Adaptation of a Speech Recogniser Using Entropy Minimisation and Speaker Codes](https://arxiv.org/abs/2506.10653)
...
<details>
  <summary>Details</summary>
Motivation: 语音识别器通常在特定环境下表现最佳，但适应新环境（如新说话人）时数据不足且未标记，导致自适应不鲁棒。

Method: 1. 提出基于完整假设的条件熵损失函数，替代单错误伪标签的交叉熵；2. 引入短向量形式的说话人编码，减少数据需求。

Result: 在远场噪声增强的Common Voice数据集上，1分钟自适应数据使词错误率相对降低20%，10分钟时提升至29%。

Conclusion: 该方法通过多假设和说话人编码，显著提升了少量数据下的自适应鲁棒性。
...
### [231] [Augmenting Large Language Models with Static Code Analysis for Automated Code Quality Improvements](https://arxiv.org/abs/2506.10330)
*Seyed Moein Abtahi,Akramul Azim*

Main category: cs.SE

TL;DR: 该研究通过将大型语言模型（如GPT-3.5 Turbo和GPT-4o）集成到软件开发工作流中，实现了代码问题的自动化检测与修复。结合静态代码分析框架和检索增强生成（RAG），显著减少了代码问题。


<details>
  <summary>Details</summary>
Motivation: 提高代码质量、简化开发流程并减少资源消耗。

Method: 使用静态代码分析框架检测问题，结合LLMs和RAG进行自动化修复，并通过“代码比较应用”解决LLM幻觉问题。

Result: 后续扫描显示代码问题显著减少。

Conclusion: 结合LLMs、静态分析和RAG能有效提升代码质量并优化开发流程。
...
### [232] [Formalising Software Requirements using Large Language Models](https://arxiv.org/abs/2506.10704)
*Arshad Beg,Diarmuid O'Donoghue,Rosemary Monahan*

Main category: cs.SE

TL;DR: VERIFAI项目旨在通过自然语言处理和人工智能技术，自动生成形式化规范并追踪需求，从软件设计到实现和验证阶段。


<details>
  <summary>Details</summary>
Motivation: 解决形式化规范在追踪和验证中的挑战，提高软件开发的效率和准确性。

Method: 结合自然语言处理、本体论、相似性重用、大语言模型和人工智能技术。

Result: 项目探索了多种技术方法，以实现需求的自动生成和追踪。

Conclusion: VERIFAI项目为形式化规范的生成和需求追踪提供了创新解决方案。
...
### [233] [What Users Value and Critique: Large-Scale Analysis of User Feedback on AI-Powered Mobile Apps](https://arxiv.org/abs/2506.10785)
*Vinaik Chhetri,Krishna Upadhyay,A. B. Siddique,Umar Farooq*

Main category: cs.SE

TL;DR: 本文通过大规模用户反馈分析，揭示了用户对AI移动应用的感知和评价，开发了一个高精度的多阶段分析流程，提取了超过一百万条情感-主题对，并发现了用户关注的核心主题。
...
Result: 提取了18个正面和15个负面用户主题，发现用户关注的核心主题包括生产力、可靠性和技术故障等。
...
### [234] [SWE-Factory: Your Automated Factory for Issue Resolution Training Data and Evaluation Benchmarks](https://arxiv.org/abs/2506.10954)
...
Method: SWE-Factory整合了三个自动化组件：SWE-Builder（多代理系统自动化环境搭建）、标准化退出码评分方法和自动化fail2pass验证。

Result: 实验表明，SWE-Factory能高效构建有效任务实例（如GPT-4.1-mini下$0.045/实例），评分准确率100%，验证精度0.92，召回率1.00。

Conclusion: SWE-Factory能加速高质量数据集的收集，为LLMs的训练和评估提供支持。
...
### [235] [Encoding call-by-push-value in the pi-calculus](https://arxiv.org/abs/2506.10584)
*Benjamin Bennetzen,Nikolaj Rossander Kristensen,Peter Buus Steffensen*

Main category: cs.LO

TL;DR: 该论文定义了一种将Levy的按值调用lambda演算（CBPV）编码到pi演算的方法，并证明了其编码的完备性和正确性。通过非形式化的手工证明，展示了编码在内部pi演算（pi-i-calculus）中的优势，并满足Gorla提出的五项编码标准。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于解决在形式化中使用de Bruijn索引的挑战，并通过内部pi演算简化双模拟问题，同时验证编码的合理性。

Method: 方法包括在pi-i-calculus、异步多态pi演算和局部pi演算中对CBPV进行编码，并通过手工证明和Coq形式化验证其完备性和正确性。

Result: 结果表明编码在pi-i-calculus中是完备且正确的，且满足Gorla的标准，同时展示了与Milner编码的相似性。

Conclusion: 论文结论指出编码的有效性，并开始通过Coq形式化进一步验证，尽管部分引理尚未完全形式化证明，但其合理性通过手工证明得到支持。
...
### [236] [StepProof: Step-by-step verification of natural language mathematical proofs](https://arxiv.org/abs/2506.10558)
...
Result: 实验结果表明，StepProof在证明成功率和效率上优于传统方法，且对自然语言证明的微小调整可进一步提升性能。
...
### [237] [Ground Reaction Force Estimation via Time-aware Knowledge Distillation](https://arxiv.org/abs/2506.10265)
...
<details>
  <summary>Details</summary>
Motivation: 传统跑步机测量GRF成本高且不便携，而便携式鞋垫传感器虽低成本但精度不足，需改进其准确性。

Method: 采用时间感知知识蒸馏框架，利用小批量数据中的相似性和时序特征，捕捉特征间的互补关系和数据的时序特性。

Result: 实验表明，该框架在GRF估计上优于现有基线方法。

Conclusion: 时间感知知识蒸馏框架能有效提升便携式传感器数据的GRF估计精度，具有实际应用潜力。
...
Result: 研究发现标记化方法在重建、下游任务和声学语言建模中的表现，并分析了权衡。

Conclusion: 总结了关键限制、实际考虑和开放挑战，为未来研究提供指导。
...
TL;DR: 本文研究了如何通过架构设计优化音频-LLM的跨模态信息传递，提出了延迟音频集成、仅通过注意力子模块探测音频表示以及集成多样化音频编码器的方法，最终实现了10%到60%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 尽管音频-LLM在应用方面进展迅速，但其底层机制（如音频编码器与LLM之间的语义表示传递）仍未充分探索。本文旨在通过架构设计优化这一过程。

Method: 从标准音频-LLM架构出发，提出并评估了三种改进：延迟音频集成、仅通过注意力子模块探测音频表示、集成多样化音频编码器。所有假设在560万音频-文本对数据集上进行了三阶段训练验证。

Result: 实验表明，提出的改进方法显著提升了性能，最终架构相比基线实现了10%到60%的相对改进。
...
### [241] [BNMusic: Blending Environmental Noises into Personalized Music](https://arxiv.org/abs/2506.10754)
...
<details>
  <summary>Details</summary>
Motivation: 传统声学掩蔽技术因主导声音与噪音不匹配（如节奏错位）需要过度增加音量，效果有限。受跨模态生成技术启发，研究提出通过生成个性化音乐来更有效地掩盖噪音。

Method: 提出BNMusic框架，分两阶段：1）基于用户文本提示生成包含噪音音乐特征的mel频谱音乐；2）自适应放大生成音乐以增强掩蔽效果，同时保持音质。

Result: 在MusicBench、EPIC-SOUNDS和ESC-50数据集上的实验表明，BNMusic能有效生成节奏对齐、适应性放大的音乐，显著减少噪音感知。

Conclusion: BNMusic通过生成个性化音乐，显著提升了声学掩蔽效果，改善了整体听觉体验。
...
### [242] [Learning Chaotic Dynamics with Neuromorphic Network Dynamics](https://arxiv.org/abs/2506.10773)
*Yinhao Xu,Georg A. Gottwald,Zdenka Kuncic*

Main category: cond-mat.dis-nn

TL;DR: 研究探讨了如何用神经形态网络学习和建模动态系统，通过电路模拟和储层计算框架优化输入电压和电极配置，以实现复杂动态系统的预测。


<details>
  <summary>Details</summary>
Motivation: 探索如何利用神经形态网络的物理特性进行动态系统的计算和学习，以优化外部控制参数。

Method: 基于忆阻元件的复杂电路模拟神经形态网络，通过储层计算框架对多元混沌时间序列进行自主预测。

Result: 输入电压最大化忆阻组件动态范围时，非线性动态响应最佳；增加电极覆盖会抑制不利于学习的非线性响应。

Conclusion: 研究为优化神经形态网络设备学习复杂动态系统提供了实用方法，仅需调整外部控制参数。
...
TL;DR: 论文提出了一种名为MAGIC的多模态心脏子结构分割方法，基于nnU-Net架构，适用于不同模态和重叠结构的分割，性能优于其他模型。


<details>
  <summary>Details</summary>
Motivation: 心脏子结构在胸部放射治疗中至关重要，但现有深度学习方法在多模态和重叠结构分割上缺乏通用性。

Method: MAGIC通过复制的编码和解码分支实现多模态分割，使用nnU-Net架构，并在Sim-CT、MR-Linac和CCTA三种模态上训练和验证。

Result: MAGIC在57%的案例中表现优于其他模型，平均DSC分数为0.75（Sim-CT）、0.68（MR-Linac）和0.80（CCTA）。

Conclusion: MAGIC是一种轻量级、高效的多模态分割解决方案，简化了计算需求并提高了临床应用的灵活性。
...
TL;DR: 提出了一种将大型语言模型（LLM）与交互式Lisp环境结合的新架构，支持LLM通过REPL动态定义、调用和演化工具。


<details>
  <summary>Details</summary>
Motivation: 探索如何将符号编程与神经语言生成结合，以实现状态化外部存储、反射式编程和动态工具创建。

Method: 通过中间件层拦截Lisp表达式，嵌入生成过程中，实现与实时REPL的程序化交互。
...
### [245] [A multi-scale loss formulation for learning a probabilistic model with proper score optimisation](https://arxiv.org/abs/2506.10868)
*Simon Lang,Martin Leutbecher,Pedro Maciel*

Main category: physics.ao-ph

TL;DR: 多尺度损失公式在概率机器学习天气预测模型中的应用效果评估。


<details>
  <summary>Details</summary>
Motivation: 探讨多尺度损失对天气预测模型中小尺度变化的约束能力及其对预测技能的影响。

Method: 在AIFS-CRPS模型中测试多尺度损失，该模型通过优化afCRPS进行训练。

Result: 多尺度损失能更好地约束小尺度变化，且不影响预测技能。
...
### [246] [Extended Creativity: A Conceptual Framework for Understanding Human-AI Creative Relations](https://arxiv.org/abs/2506.10249)
*Andrea Gaggioli,Sabrina Bartolotta,Andrea Ubaldi,Katusha Gerardini,Eleonora Diletta Sarcinella,Alice Chirico*

Main category: cs.HC

TL;DR: AI can enhance human creativity through three modes: Support, Synergy, and Symbiosis, defined by AI autonomy and perceived agency, impacting creativity levels from problem-solving to innovation.


<details>
  <summary>Details</summary>
Motivation: To clarify how AI can effectively enhance human creativity by exploring different modes of AI-human interaction.

Method: Adopting a distributed creativity perspective, the paper identifies three primary modes (Support, Synergy, Symbiosis) based on AI autonomy and perceived agency, and examines their impact on creativity levels.

Result: The study defines how each AI-human interaction mode influences creativity, from everyday tasks to paradigm-shifting innovations, and discusses theoretical, ethical, and design implications.

Conclusion: Understanding AI's role in creativity through these modes provides a framework for future research and practical applications, balancing technical and ethical considerations.
...
### [247] [The Role of Generative AI in Facilitating Social Interactions: A Scoping Review](https://arxiv.org/abs/2506.10927)
*T. T. J. E. Arets,G. Perugia,M. Houben,W. A. IJsselsteijn*

Main category: cs.HC

TL;DR: 这篇论文探讨了生成式AI（GAI）技术如何影响社交互动，总结了30项研究的关键趋势，并强调了参与式设计和公平性问题。


<details>
  <summary>Details</summary>
Motivation: 社交连接的减少对心理健康和福祉构成威胁，而GAI技术在社交应用中的使用日益增多，但其影响尚不明确。

Method: 通过分析2020年以来的30项研究，总结了GAI应用的设计目标、社交互动形式及评估方法。

Result: 发现GAI在故事讲述、情感技能训练等领域有潜力，但需关注文化偏见和可访问性等社会伦理问题。

Conclusion: GAI支持个性化和动态互动，但需更注重公平设计和包容性评估。
...
### [248] [Equitable Mechanism Design for Facility Location](https://arxiv.org/abs/2506.10460)
*Toby Walsh*

Main category: cs.GT

TL;DR: 论文研究了在设施选址问题中如何通过策略证明机制最大化公平性，使用基尼指数衡量公平性，并证明了策略证明机制无法限制基尼指数的近似比。转而提出计算互补基尼指数的近似比，并探讨了确定性和随机性机制的表现。同时，研究了纳什福利作为公平妥协的表现。


<details>
  <summary>Details</summary>
Motivation: 研究如何在设施选址问题中设计策略证明机制以最大化公平性，同时避免策略性操纵。

Method: 使用基尼指数衡量公平性，证明策略证明机制无法限制其近似比，转而分析互补基尼指数和纳什福利的近似比。

Result: 证明了策略证明机制无法限制基尼指数的近似比，但可以通过互补基尼指数和纳什福利实现更好的公平性表现。

Conclusion: 互补基尼指数和纳什福利为设施选址问题中的公平性提供了可行的替代方案。
...
### [249] [Leveraging LLMs for Mission Planning in Precision Agriculture](https://arxiv.org/abs/2506.10093)
*Marcos Abel Zuzuárregui,Stefano Carpin*

Main category: cs.RO

TL;DR: 论文提出了一种利用大型语言模型（如ChatGPT）的端到端系统，使非技术用户能够通过自然语言指令为自主机器人分配复杂的数据收集任务。


<details>
  <summary>Details</summary>
Motivation: 机器人系统和人工智能在精准农业中潜力巨大，但适应多样化任务仍具挑战性，尤其是非技术用户难以操作。

Method: 系统将任务计划编码为IEEE任务规范标准，并通过ROS2节点执行，连接高级任务描述与现有ROS库。

Result: 实验展示了大型语言模型在此场景中的优势与局限（如空间推理和复杂路径规划），并验证了所提解决方案的有效性。

Conclusion: 该系统为非技术用户提供了便捷的机器人任务分配方式，同时克服了大型语言模型在特定任务中的局限性。
...
### [250] [One For All: LLM-based Heterogeneous Mission Planning in Precision Agriculture](https://arxiv.org/abs/2506.10106)
*Marcos Abel Zuzuárregui,Mustafa Melih Toslak,Stefano Carpin*

Main category: cs.RO

TL;DR: 论文提出了一种基于自然语言的机器人任务规划系统，利用大语言模型和预定义原语，帮助非技术用户通过简单语言控制异构机器人，简化农业任务规划。


<details>
  <summary>Details</summary>
Motivation: 人工智能在精准农业中的应用虽然提高了效率，但技术复杂性对非技术用户构成挑战。本文旨在通过自然语言接口降低使用门槛。

Method: 系统利用大语言模型（LLMs）和预定义原语，将人类语言翻译为中间描述，供不同机器人平台执行。用户无需编写代码即可规划复杂任务。
...
### [251] [A Navigation Framework Utilizing Vision-Language Models](https://arxiv.org/abs/2506.10172)
*Yicheng Duan,Kaiyu tang*

Main category: cs.RO

TL;DR: 提出了一种模块化的视觉语言导航框架，通过解耦视觉语言理解和动作规划，结合轻量级规划逻辑和冻结的视觉语言模型，实现了灵活、快速的导航。


<details>
  <summary>Details</summary>
Motivation: 解决现有大型视觉语言模型在计算成本和实时部署方面的挑战，同时提升导航系统的灵活性和适应性。

Method: 采用模块化设计，结合冻结的视觉语言模型（Qwen2.5-VL-7B-Instruct）和轻量级规划逻辑，利用提示工程、结构化历史管理和双帧视觉输入策略。

Result: 在Room-to-Room基准测试中初步验证了框架的有效性，但在未见环境下的泛化能力仍有挑战。
...
### [252] [Using Language and Road Manuals to Inform Map Reconstruction for Autonomous Driving](https://arxiv.org/abs/2506.10317)
...
<details>
  <summary>Details</summary>
Motivation: 车道拓扑预测是自动驾驶导航中的关键任务，而道路环境信息的准确理解对此至关重要。作者发现这些信息通常遵循自然语言编码的惯例，如反映道路结构的设计规范和捕捉道路功能的路名。

Method: 通过将OSM地图的结构化道路元数据和道路设计手册中的车道宽度先验与道路中心线编码结合，对SMERF模型进行了轻量级增强。

Result: 在两个地理多样化的复杂交叉口场景中评估，该方法在车道和交通元素检测及其关联方面均表现出改进。通过四种拓扑感知指标全面评估模型性能。
...
### [253] [EmbodiedGen: Towards a Generative 3D World Engine for Embodied Intelligence](https://arxiv.org/abs/2506.10600)
...
Result: 生成的3D资产可直接导入物理模拟引擎，支持细粒度物理控制，满足具身智能研究的训练和评估需求。

Conclusion: EmbodiedGen解决了3D资产生成的成本和真实性问题，为具身智能研究提供了可扩展的工具。
...
### [254] [Grounded Vision-Language Navigation for UAVs with Open-Vocabulary Goal Understanding](https://arxiv.org/abs/2506.10756)
*Yuhang Zhang,Haosheng Yu,Jiaping Xiao,Mir Feroskhan*

Main category: cs.RO

TL;DR: VLFly框架通过结合大型语言模型和视觉语言模型，解决了无人机在视觉与语言导航中的泛化问题和连续动作空间需求，无需额外传感器即可实现实时控制。


<details>
  <summary>Details</summary>
Motivation: 视觉与语言导航（VLN）在自主机器人领域存在两大瓶颈：对分布外环境的泛化能力和依赖固定离散动作空间。VLFly旨在解决这些问题，为无人机提供语言引导的飞行能力。

Method: VLFly包含三个模块：基于LLM的指令编码器、基于VLM的目标检索器和航点规划器，通过单目摄像头输入生成连续速度命令。

Result: 在多样化仿真环境中，VLFly无需微调即优于所有基线方法，并在真实世界的室内外环境中展示了强大的开放词汇目标理解和泛化导航能力。

Conclusion: VLFly通过结合语言和视觉模型，实现了无人机在复杂环境中的高效导航，为VLN领域提供了新的解决方案。
...
### [255] [Eye, Robot: Learning to Look to Act with a BC-RL Perception-Action Loop](https://arxiv.org/abs/2506.10968)
*Justin Kerr,Kush Hari,Ethan Weber,Chung Min Kim,Brent Yi,Tyler Bonnen,Ken Goldberg,Angjoo Kanazawa*

Main category: cs.RO

TL;DR: EyeRobot是一个通过强化学习训练机械眼球实现任务驱动的注视行为的机器人系统，通过手眼协同完成任务。


<details>
  <summary>Details</summary>
Motivation: 人类通过主动观察来完成任务，因此设计了一个任务驱动的机器人系统以实现类似行为。

Method: 开发可旋转的机械眼球，收集360度摄像头数据，在模拟环境中训练注视策略，并通过BC-RL循环联合训练手和眼的策略。

Result: EyeRobot在五个全景工作空间任务中表现出有效的手眼协同行为，能够在大工作空间中完成任务。

Conclusion: EyeRobot展示了任务驱动的注视行为如何有效提升机器人在大工作空间中的操作能力。
...
### [256] [Data-Driven Prediction of Dynamic Interactions Between Robot Appendage and Granular Material](https://arxiv.org/abs/2506.10875)
*Guanjin Wang,Xiangxue Zhao,Shapour Azarm,Balakumar Balachandran*

Main category: cs.RO

TL;DR: 提出了一种数据驱动的建模方法，结合降维、代理建模和数据同化技术，显著减少计算时间，并在预测精度上与高保真物理模拟相当甚至更优。


<details>
  <summary>Details</summary>
Motivation: 研究机器人运动与颗粒地形交互的基本原理，为机器人在复杂地形中的导航和探索提供支持。

Method: 集成降维（ST-HOSVD）、代理建模（高斯过程）和数据同化技术（ROPF），利用离线高保真模拟数据和稀疏实验数据进行在线建模。

Result: 计算时间显著减少，预测精度与模拟相当；结合实验数据后，长期预测优于仅用模拟数据。

Conclusion: 该方法在机器人导航和复杂地形探索中具有潜力，且能推广到其他案例。
...
### [257] [Agentic Semantic Control for Autonomous Wireless Space Networks: Extending Space-O-RAN with MCP-Driven Distributed Intelligence](https://arxiv.org/abs/2506.10925)
...
TL;DR: 论文提出了一种基于语义代理层的扩展方案，用于提升月球表面无线通信系统的自主性和适应性。


<details>
  <summary>Details</summary>
Motivation: 月球表面无线通信系统需要满足自主性、抗干扰性和环境适应性等严格需求，现有Space-O-RAN模型的决策逻辑仅依赖静态策略且缺乏语义集成。
...
Conclusion: 该扩展方案显著提升了月球无线通信系统的智能化和适应性。
...
### [258] [FastFLUX: Pruning FLUX with Block-wise Replacement and Sandwich Training](https://arxiv.org/abs/2506.10035)
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型（如FLUX）参数庞大，导致推理速度慢、内存占用高，现有加速方法性能下降明显且训练成本高。
...
Conclusion: FastFLUX通过架构优化和训练策略，有效解决了现有模型的推理效率问题。
...
### [259] [Ambient Diffusion Omni: Training Good Models with Bad Data](https://arxiv.org/abs/2506.10038)
*Giannis Daras,Adrian Rodriguez-Munoz,Adam Klivans,Antonio Torralba,Constantinos Daskalakis*

Main category: cs.GR

TL;DR: 论文提出了一种名为Ambient Diffusion Omni的框架，利用低质量、合成和分布外图像提升扩散模型的质量。


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型依赖高质量数据集，但低质量图像中蕴含的信息被忽视，论文旨在挖掘这些被丢弃数据的价值。

Method: 通过利用自然图像的光谱幂律衰减和局部性特性，设计了一个简单而原则性的框架，能够从所有可用图像中提取信号。

Result: 实验验证了框架在合成损坏图像上的有效性，并在ImageNet上取得了最优FID，显著提升了文本到图像生成的质量和多样性。
...
### [260] [Learning-based density-equalizing map](https://arxiv.org/abs/2506.10027)
...
### [261] [Low-Barrier Dataset Collection with Real Human Body for Interactive Per-Garment Virtual Try-On](https://arxiv.org/abs/2506.10468)
*Zaiqiang Wu,Yechen Li,Jingyuan Liu,Yuki Shibata,Takayuki Hori,I-Chao Shen,Takeo Igarashi*

Main category: cs.GR

TL;DR: 论文提出了一种低成本、基于真人身体的虚拟试衣方法，解决了现有方法视角受限、实时性差以及服装与人体对齐不准确的问题。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试衣方法多局限于正面视角且缺乏实时性，而基于单件服装的方法虽有所改进，但仍面临数据采集成本高、人体变形不自然及服装对齐不准确的问题。

Method: 采用真人身体采集服装数据集，避免使用昂贵的机器人模型，并引入混合人物表示（结合简化DensePose图）以提升服装与人体对齐精度。

Result: 通过定性和定量评估，证明了该方法在图像质量和时间一致性上的优越性，用户研究显示其对服装购买决策有帮助。
...
### [262] [Edit360: 2D Image Edits to 3D Assets from Any Angle](https://arxiv.org/abs/2506.10507)
*Junchao Huang,Xinting Hu,Zhuotao Tian,Shaoshuai Shi,Li Jiang*

Main category: cs.GR

TL;DR: Edit360是一个无需调整的框架，通过视频扩散模型实现多视角一致的3D编辑，解决了现有方法在视角限制和一致性上的不足。
...
Method: Edit360基于视频扩散模型，通过锚点视图选择和编辑传播机制，在潜在和注意力空间中实现多视角信息对齐。

Result: Edit360能够生成高质量的多视角序列，支持可定制的3D内容创建。

Conclusion: Edit360为3D编辑提供了一种灵活且一致的解决方案，扩展了扩散模型的应用范围。
...
### [263] [Transformer IMU Calibrator: Dynamic On-body IMU Calibration for Inertial Motion Capture](https://arxiv.org/abs/2506.10580)
*Chengxu Zuo,Jiawei Huang,Xiao Jiang,Yuan Yao,Xiangren Shi,Rui Cao,Xinyu Yi,Feng Xu,Shihui Guo,Yipeng Qin*

Main category: cs.GR

TL;DR: 提出了一种新型的动态校准方法，打破了IMU校准中的绝对静态假设，实现了实时估计RG'G和RBS，扩展了稀疏惯性运动捕捉系统的应用场景。


<details>
  <summary>Details</summary>
Motivation: 传统IMU校准方法依赖于绝对静态假设（RG'G和RBS恒定），限制了其应用场景。本文旨在通过动态校准方法解决这一问题。

Method: 基于两个宽松假设（RG'G和RBS在短时间内变化微小；IMU读数在时间窗口内多样化），利用Transformer模型学习RG'G、RBS与IMU读数的映射关系，并设计校准触发器确保假设成立。

Result: 首次实现了隐式IMU校准（无需显式校准过程），并支持稀疏IMU的长期精确运动捕捉。

Conclusion: 该方法突破了传统限制，为稀疏IMU系统提供了更灵活、高效的校准方案。
...
Conclusion: 基于STP的CP方法有效解决了填充问题，展示了在信号处理中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究多关注控制和观测架构，但相机位置对图像质量的影响被忽视，尤其在制造环境中噪声水平因位置不同而变化。

Method: 设计相机移动策略算法，结合图像平均技术，高效搜索最优或次优位置，确保有限能量下的观测精度。

Result: 模拟制造应用显示，该算法在有限能量下显著提升观测精度，且保留高频信息。

Conclusion: 该算法有效优化相机位置选择，提升眼到手配置的观测质量，适用于自动化制造任务。
...
### [266] [From Threat to Tool: Leveraging Refusal-Aware Injection Attacks for Safety Alignment](https://arxiv.org/abs/2506.10020)
...
<details>
  <summary>Details</summary>
Motivation: 传统的人类标注偏好数据成本高且耗时，现有合成数据方法复杂，需要更简单有效的解决方案。
...
### [267] [LLMs Caught in the Crossfire: Malware Requests and Jailbreak Challenges](https://arxiv.org/abs/2506.10022)
*Haoyang Li,Huan Gao,Zhiyuan Zhao,Zhiyu Lin,Junyu Gao,Xuelong Li*

Main category: cs.CR

TL;DR: 论文提出了MalwareBench基准数据集，用于评估大型语言模型（LLMs）在代码生成中对越狱攻击的鲁棒性，发现主流LLMs对恶意代码生成的拒绝能力有限。


<details>
  <summary>Details</summary>
Motivation: 研究填补了LLMs在代码生成中越狱攻击脆弱性研究的空白，关注其安全挑战。

Method: 基于320个手工设计的恶意代码生成需求，构建包含3,520个越狱提示的MalwareBench数据集，覆盖11种越狱方法和29种代码功能类别。
...
### [268] [Private Memorization Editing: Turning Memorization into a Defense to Strengthen Data Privacy in Large Language Models](https://arxiv.org/abs/2506.10024)
*Elena Sofia Ruzzetti,Giancarlo A. Xompero,Davide Venditti,Fabio Massimo Zanzotto*

Main category: cs.CR

TL;DR: 论文提出了一种名为PME的方法，利用LLMs的记忆能力来防止隐私数据泄露，通过编辑模型对训练数据的记忆来减少PII泄漏。


<details>
  <summary>Details</summary>
Motivation: LLMs可能记忆并泄露个人隐私信息（PII），需要一种方法来防止这种隐私泄漏。

Method: 提出Private Memorization Editing（PME），通过检测并编辑模型记忆的PII来减少隐私泄漏。

Result: PME有效减少了PII泄漏，在某些情况下甚至将隐私攻击准确率降至零。

Conclusion: PME将LLMs的记忆能力转化为隐私防御工具，提升了模型对隐私攻击的鲁棒性。
...
### [269] [Evaluation empirique de la sécurisation et de l'alignement de ChatGPT et Gemini: analyse comparative des vulnérabilités par expérimentations de jailbreaks](https://arxiv.org/abs/2506.10029)
*Rafaël Nouailles*
...
### [270] [Safeguarding Multimodal Knowledge Copyright in the RAG-as-a-Service Environment](https://arxiv.org/abs/2506.10030)
*Tianyu Chen,Jian Lou,Wenjie Wang*

Main category: cs.CR

TL;DR: AQUA是首个针对多模态RAG系统中图像知识保护的水印框架，通过嵌入语义信号实现高效、有效且不可感知的版权追踪。


<details>
  <summary>Details</summary>
Motivation: 随着RAG发展为共享知识库的服务平台（Rag-as-a-Service），保护贡献数据的版权变得至关重要，但现有水印方法仅针对文本知识，图像知识未受保护。
...
### [271] [Disclosure Audits for LLM Agents](https://arxiv.org/abs/2506.10171)
*Saswat Das,Jameson Sandler,Ferdinando Fioretto*

Main category: cs.CR

TL;DR: 论文提出了一种名为CMPL的对话隐私审计框架，通过多轮交互测试揭示语言模型代理的隐私泄露风险。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型代理在应用中需要持续访问敏感数据，增加了未经授权披露的风险，现有单轮防御措施不足以应对。

Method: 提出CMPL框架，采用迭代探测策略模拟多轮对话，系统性地发现隐私漏洞。

Result: 评估表明CMPL能够揭示现有单轮防御无法检测的隐私风险，并提供量化风险指标和开放基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有的对抗攻击研究在生成有害内容或绕过安全机制方面存在局限性，缺乏可靠的工具评估T2I模型的安全性。

Method: 结合监督微调和强化学习，通过多奖励信号指导LLM生成既能绕过安全机制又具有高毒性的对抗提示。

Result: 生成的对抗提示在黑盒攻击中表现出色，揭示了商业T2I生成器的实际安全漏洞。

Conclusion: GenBreak为评估T2I模型的安全性提供了有效工具，揭示了其潜在的高风险漏洞。
...
### [273] [Prompt Attacks Reveal Superficial Knowledge Removal in Unlearning Methods](https://arxiv.org/abs/2506.10236)
*Yeonwoo Jang,Shariqah Hossain,Ashwin Sreevatsa,Diogo Cruz*

Main category: cs.CR

TL;DR: 研究发现某些机器学习遗忘方法在简单提示攻击下可能失效，部分方法如ELM仍易受攻击，而RMU和TAR表现稳健。结果挑战了现有遗忘效果的假设，并提出了更可靠的评估框架需求。


<details>
  <summary>Details</summary>
Motivation: 评估现有机器学习遗忘方法的有效性，特别是在面对提示攻击时的表现，以验证其是否真正移除知识而非仅抑制输出。

Method: 系统评估了八种遗忘技术，采用输出、logit和探针分析，测试未学习知识的可检索性。

Result: 部分方法（如ELM）在特定提示攻击下表现脆弱（如恢复57.3%准确率），而RMU和TAR表现稳健。logit分析显示未学习模型未通过格式化答案隐藏知识。

Conclusion: 研究揭示了遗忘方法的局限性，强调需要更可靠的评估框架来区分真正知识移除与表面输出抑制。
...
### [274] [SOFT: Selective Data Obfuscation for Protecting LLM Fine-tuning against Membership Inference Attacks](https://arxiv.org/abs/2506.10424)
...
TL;DR: 该论文研究了微调大型语言模型（LLMs）时面临的隐私风险，特别是成员推理攻击（MIAs），并提出了一种名为SOFT的新型防御技术，以平衡隐私保护和模型性能。
...
Result: 实验表明，SOFT能有效降低隐私风险，并在多个领域和模型规模中保持竞争力。
...
### [275] [Specification and Evaluation of Multi-Agent LLM Systems -- Prototype and Cybersecurity Applications](https://arxiv.org/abs/2506.10467)
*Felix Härer*

Main category: cs.CR

TL;DR: 论文探讨了基于多智能体LLM系统的潜力，提出了系统架构和原型，并通过网络安全任务验证了其可行性。


<details>
  <summary>Details</summary>
Motivation: 现有研究对LLMs、推理技术和应用的联合规范及组合应用探索不足，需定义多智能体LLM系统的规范以系统性评估其潜力。

Method: 扩展了先前研究的系统架构和原型，引入多智能体系统的规范，并通过网络安全任务进行测试。

Result: 测试结果表明，基于OpenAI和DeepSeek的LLM智能体能正确完成问答、服务器安全和网络安全任务。

Conclusion: 多智能体LLM系统在特定领域应用中具有潜力，需进一步探索其规范和评估方法。
...
### [276] [Secure Data Access in Cloud Environments Using Quantum Cryptography](https://arxiv.org/abs/2506.10028)
*S. Vasavi Venkata Lakshmi,Ziaul Haque Choudhury*

Main category: cs.CR

TL;DR: 该研究利用量子密码学（如QKD和BB84协议）结合量子一次性密码本（QOTP）来增强云环境中的数据安全性，抵御未来量子计算机的威胁。
...
Result: 该方法为云数据提供了强大的安全保障，即使面对量子计算机攻击也能有效防御。
...
### [277] [SoK: Evaluating Jailbreak Guardrails for Large Language Models](https://arxiv.org/abs/2506.10597)
*Xunguang Wang,Zhenlan Ji,Wenxuan Wang,Zongjie Li,Daoyuan Wu,Shuai Wang*

Main category: cs.CR

TL;DR: 本文提出了一种针对大型语言模型（LLM）越狱攻击防护措施的系统性分析，包括多维度分类法和评估框架。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM取得了显著进展，但其部署暴露了安全漏洞，尤其是越狱攻击绕过安全机制的问题。防护措施（Guardrails）作为外部防御机制，缺乏统一分类和评估框架。

Method: 提出了一个六维分类法，并引入了安全-效率-实用性评估框架，通过实验分析现有防护措施的优缺点。

Result: 研究发现现有防护措施的局限性，探讨了其在不同攻击类型中的通用性，并提供了优化防御组合的见解。

Conclusion: 本文为未来研究和开发提供了结构化基础，旨在指导LLM防护措施的进步和部署。
...
### [278] [TED-LaST: Towards Robust Backdoor Defense Against Adaptive Attacks](https://arxiv.org/abs/2506.10722)
*Xiaoxing Mo,Yuxuan Cheng,Nan Sun,Leo Yu Zhang,Wei Luo,Shang Gao*

Main category: cs.CR

TL;DR: TED-LaST 是一种新的防御策略，通过标签监督的动态跟踪和自适应层强调，增强了对自适应后门攻击的检测能力。
...
Method: TED-LaST 引入了标签监督的动态跟踪和自适应层强调，以识别传统 TED 无法检测的隐蔽威胁。

Result: 在多个数据集和模型架构上的实验表明，TED-LaST 能有效对抗复杂的后门攻击。

Conclusion: TED-LaST 为后门检测设立了新标准，显著提升了 DNN 的安全性。
...
### [279] [ME: Trigger Element Combination Backdoor Attack on Copyright Infringement](https://arxiv.org/abs/2506.10776)
*Feiyu Yang,Siyuan Liang,Aishan Liu,Dacheng Tao*

Main category: cs.CR

TL;DR: 论文提出了一种基于SilentBadDiffusion（SBD）的多元素（ME）攻击方法，通过增加每个毒样本的视觉-文本元素数量来增强攻击能力，同时引入离散余弦变换（DCT）保持隐蔽性。


<details>
  <summary>Details</summary>
Motivation: 生成扩散模型（如Stable Diffusion）可能被用于版权侵权攻击，但现有数据集资源有限且攻击效果不理想。

Method: 提出ME攻击方法，结合DCT技术，增强攻击能力并保持隐蔽性。

Result: 在两个新数据集上，CIR/FAE分别达到16.78%/39.50和51.20%/23.60，优于基准数据集。在低采样率下，ME和DCT表现优于原始SBD。

Conclusion: ME攻击方法显著提升了攻击效果，尤其在资源受限条件下表现优异。
...
### [280] [Monitoring Decomposition Attacks in LLMs with Lightweight Sequential Monitors](https://arxiv.org/abs/2506.10949)
*Chen Yueh-Han,Nitish Joshi,Yulin Chen,Maksym Andriushchenko,Rico Angell,He He*

Main category: cs.CR

TL;DR: 当前LLM安全防御在分解攻击下失效，恶意目标被分解为无害子任务绕过拒绝。现有浅层安全对齐技术仅检测即时提示的危害，无法推理长期意图。提出外部监控器，观察更高粒度的对话。构建最大多样数据集验证攻击有效性（87%成功率）。提出轻量级顺序监控框架，防御成功率达93%，成本降低90%，延迟减少50%。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全防御无法应对分解攻击，因浅层对齐技术仅关注即时提示，忽略长期恶意意图。需更高粒度监控以识别隐蔽攻击。

Method: 提出外部监控器，构建多样化数据集验证攻击效果。设计轻量级顺序监控框架，实时评估子任务。

Result: 分解攻击平均成功率87%（GPT-4o）。轻量监控器防御成功率93%，成本降90%，延迟减50%。
...
### [281] [An Analysis of Datasets, Metrics and Models in Keyphrase Generation](https://arxiv.org/abs/2506.10346)
*Florian Boudin,Akiko Aizawa*

Main category: cs.IR

TL;DR: 本文综述了50多篇关键词生成研究论文，总结了最新进展、局限性和开放挑战，并指出了当前评估实践中的问题，同时发布了一个基于PLM的模型以促进未来研究。


<details>
  <summary>Details</summary>
Motivation: 关键词生成任务在过去几年中受到广泛关注，但缺乏对现有工作的系统综述和分析，因此本文旨在填补这一空白。

Method: 通过分析50多篇相关研究论文，总结了关键词生成领域的最新进展、局限性和挑战，并提出了一个基于预训练语言模型（PLM）的关键词生成模型。

Result: 研究发现当前评估实践中存在数据集相似性和指标计算不一致等问题，导致性能被高估。同时，发布了PLM模型以支持未来研究。

Conclusion: 本文为关键词生成领域提供了全面的综述，指出了当前问题，并通过发布模型推动了未来研究的发展。
...
### [282] [Towards Understanding Bias in Synthetic Data for Evaluation](https://arxiv.org/abs/2506.10301)
*Hossein A. Rahmani,Varsha Ramineni,Nick Craswell,Bhaskar Mitra,Emine Yilmaz*

Main category: cs.IR

TL;DR: 论文探讨了使用大型语言模型（LLMs）生成合成测试集在信息检索系统评估中的可靠性，并分析了潜在的偏见问题。


<details>
  <summary>Details</summary>
Motivation: 构建多样化的测试集和获取相关性标注成本高昂，LLMs生成合成数据为解决方案提供了可能，但其可靠性尚需验证。

Method: 通过LLMs生成合成查询和标注，实证分析评估结果中的偏见，并使用线性混合效应模型验证偏见的存在。

Result: 研究发现，合成测试集在绝对性能评估中可能存在显著偏见，但在相对性能比较中影响较小。

Conclusion: 合成测试集可用于系统评估，但需注意偏见问题，尤其是在绝对性能评估中。
...
### [283] [Conversational Search: From Fundamentals to Frontiers in the LLM Era](https://arxiv.org/abs/2506.10635)
*Fengran Mo,Chuan Meng,Mohammad Aliannejadi,Jian-Yun Nie*

Main category: cs.IR

TL;DR: 该教程介绍了大型语言模型（LLMs）如何革新对话式搜索，涵盖基础原理与前沿发展，适合学术界和工业界的研究者与实践者。


<details>
  <summary>Details</summary>
Motivation: 对话式搜索通过多轮交互满足复杂信息需求，而LLMs的指令遵循、内容生成和推理能力为构建智能系统提供了新机遇与挑战。

Method: 教程结合基础理论与LLMs驱动的创新主题，帮助参与者全面理解对话式搜索的核心与前沿技术。

Result: 参与者将掌握构建下一代对话式搜索系统所需的知识。

Conclusion: LLMs为对话式搜索带来革命性进展，教程旨在推动该领域的进一步发展。
...
### [284] [Contrastive Matrix Completion with Denoising and Augmented Graph Views for Robust Recommendation](https://arxiv.org/abs/2506.10658)
*Narges Nemati,Mostafa Haghir Chehreghani*

Main category: cs.IR

TL;DR: 论文提出了一种基于对比学习的矩阵补全方法（MCCL），通过提取局部子图并生成两种不同的图表示，结合注意力机制和变分自编码器提升模型的泛化能力。
...
### [285] [Precise Zero-Shot Pointwise Ranking with LLMs through Post-Aggregated Global Context Information](https://arxiv.org/abs/2506.10859)
*Kehan Long,Shasha Li,Chen Xu,Jintao Tang,Ting Wang*

Main category: cs.IR

TL;DR: 论文提出了一种结合全局比较的点式排名方法（GCCP）和高效后聚合技术（PAGC），在保持点式方法效率的同时提升其效果。


<details>
  <summary>Details</summary>
Motivation: 点式方法虽高效但忽略了文档间的比较信息，导致评分不一致和性能不佳。

Method: 提出GCCP策略，通过全局参考比较生成对比相关性分数；设计锚点文档作为查询焦点摘要，并利用PAGC技术高效整合全局上下文。

Result: 在TREC DL和BEIR基准测试中显著优于传统点式方法，且与计算资源需求更高的比较方法性能相当。

Conclusion: GCCP和PAGC有效提升了点式排名的效果，同时保持了高效性。
...
### [286] [Semantic Communication-Enabled Cloud-Edge-End-collaborative Metaverse Services Architecure](https://arxiv.org/abs/2506.10001)
...
<details>
  <summary>Details</summary>
Motivation: 随着技术发展和对新视听体验需求的增加，元宇宙受到广泛关注，但其面临高分辨率数据传输的带宽不足和延迟问题，影响了用户体验。
...
Conclusion: SC-CEE-Meta架构通过语义通信和云-边-端协作，有效解决了元宇宙中的数据传输问题，显著提升了用户体验。
...
### [287] [EQ-TAA: Equivariant Traffic Accident Anticipation via Diffusion-Based Accident Video Synthesis](https://arxiv.org/abs/2506.10002)
*Jianwu Fang,Lei-Lei Li,Zhedong Zheng,Hongkai Yu,Jianru Xue,Zhengguo Li,Tat-Seng Chua*

Main category: cs.MM

TL;DR: 论文提出了一种Attentive Video Diffusion (AVD)模型，通过生成事故视频片段解决交通场景中的事故预测问题，并引入EQ-TAA方法提升性能。


<details>
  <summary>Details</summary>
Motivation: 交通场景中事故预测（TAA）的挑战在于数据标注困难且易受背景干扰，现有方法难以准确捕捉事故的因果部分。

Method: 提出AVD模型，通过文本提示生成事故或非事故视频片段，并设计EQ-TAA方法，利用对比学习提升预测性能。

Result: 实验表明AVD和EQ-TAA在性能上优于现有方法。

Conclusion: AVD和EQ-TAA为交通事故预测提供了一种无需额外标注的有效解决方案。
...
### [288] [Immersive Multimedia Communication: State-of-the-Art on eXtended Reality Streaming](https://arxiv.org/abs/2506.10004)
*Haopeng Wang,Haiwei Dong,Abdulmotaleb El Saddik*

Main category: cs.MM

TL;DR: 本文综述了扩展现实（XR）流媒体的最新进展，包括XR定义、设备、数据需求、体验质量优化方法及未来挑战。
...
Method: 通过分析XR设备、数据特征、体验质量因素及视觉注意力优化方法，系统梳理XR流媒体的现状。

Result: 总结了XR流媒体的关键技术和优化方法，并指出了当前应用中的挑战。

Conclusion: XR流媒体技术前景广阔，但仍需解决多方面的挑战以实现更高效和用户友好的体验。
...
### [289] [HER2 Expression Prediction with Flexible Multi-Modal Inputs via Dynamic Bidirectional Reconstruction](https://arxiv.org/abs/2506.10006)
...
<details>
  <summary>Details</summary>
Motivation: 现有的HER2评估模型通常单独分析H&E或IHC图像，而临床依赖它们的协同解释。但由于工作流程复杂性和成本限制，同时获取两种模态的数据往往困难。

Method: 1) 动态分支选择器；2) 双向跨模态GAN；3) 混合训练协议。

Result: 单模态H&E预测准确率从71.44%提升至94.25%，双模态准确率达95.09%，且仅使用IHC输入时可靠性为90.28%。

Conclusion: 该框架通过弹性设计，显著提升了HER2评估的准确性和适用性，尤其适合资源有限的医疗环境。
...
### [290] [Controllable Expressive 3D Facial Animation via Diffusion in a Unified Multimodal Space](https://arxiv.org/abs/2506.10007)
*Kangwei Liu,Junwu Liu,Xiaowei Yi,Jinlin Guo,Yun Cao*

Main category: cs.MM

TL;DR: 提出了一种基于扩散模型的多模态情感3D面部动画框架，解决了单模态控制和确定性映射的局限性。
...
### [291] [Structured Graph Representations for Visual Narrative Reasoning: A Hierarchical Framework for Comics](https://arxiv.org/abs/2506.10008)
*Yi-Chun Chen*
...
<details>
  <summary>Details</summary>
Motivation: 旨在支持对视觉叙事的多层次理解，包括故事结构、角色连续性和事件发展。
...
### [292] [WDMIR: Wavelet-Driven Multimodal Intent Recognition](https://arxiv.org/abs/2506.10011)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法多关注文本分析，忽视了非语言信息中的丰富语义内容，因此需要一种新方法来更好地整合多模态信息。

Method: 1. 小波驱动的融合模块，在频域同步分解和整合视频-音频特征；2. 跨模态交互机制，从双模态逐步增强到三模态整合。

Result: 在MIntRec数据集上达到最优性能，准确率提升1.13%；小波融合模块显著提升非语言信息的语义提取能力，识别准确率增加0.41%。
...
### [293] [Multimodal Large Language Models: A Survey](https://arxiv.org/abs/2506.10016)
*Longzhen Han,Awes Mubarak,Almas Baimagambetov,Nikolaos Polatidis,Thar Baker*

Main category: cs.MM

TL;DR: 本文综述了多模态大语言模型（MLLMs）的发展，探讨了其跨模态生成能力、关键技术及挑战。


<details>
  <summary>Details</summary>
Motivation: 研究多模态大语言模型如何整合语言与其他感官模态，实现跨模态生成能力，并推动通用、自适应、可解释的多模态系统发展。

Method: 通过分类六种主要生成模态，分析自监督学习、混合专家、人类反馈强化学习和思维链提示等关键技术。

Result: 总结了关键模型、架构趋势和跨模态协同效应，同时指出评估、模块化和结构化推理等未解决问题。

Conclusion: 本文为MLLM发展提供了统一视角，并指明了未来研究方向。
...
### [294] [DUN-SRE: Deep Unrolling Network with Spatiotemporal Rotation Equivariance for Dynamic MRI Reconstruction](https://arxiv.org/abs/2506.10309)
...
Method: 提出DUN-SRE网络，通过(2+1)D等变卷积架构统一数据一致性和近端映射模块，确保时空对称性约束的严格传播。

Result: 在心脏CINE MRI数据集上，DUN-SRE实现了最先进的性能，尤其在保留旋转对称结构方面表现优异。

Conclusion: DUN-SRE通过建模时空对称性，为动态MRI重建提供了更准确的物理建模和广泛适用性。
...
### [295] [Rethinking Brain Tumor Segmentation from the Frequency Domain Perspective](https://arxiv.org/abs/2506.10142)
...
<details>
  <summary>Details</summary>
Motivation: 当前方法在分割MRI对比增强脑肿瘤区域时性能不足，主要因未充分考虑MRI特有的复杂纹理和方向变化特征。

Method: 提出HFF-Net，包括频域分解模块（FDD）、自适应拉普拉斯卷积模块（ALC）和频域交叉注意力模块（FDCA），以多尺度融合肿瘤特征。
...
### [296] [Prompt-Guided Latent Diffusion with Predictive Class Conditioning for 3D Prostate MRI Generation](https://arxiv.org/abs/2506.10230)
*Emerson P. Grabke,Masoom A. Haider,Babak Taati*

Main category: eess.IV

TL;DR: 论文提出了一种名为CCELLA的双头条件方法，结合非医学大型语言模型和病理分类，解决了医学图像生成中的数据稀缺问题，显著提升了生成质量和分类器性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像生成中数据稀缺问题限制了机器学习的发展，现有方法依赖短提示文本编码器、非医学LDM或大量数据微调，限制了性能和科学可访问性。

Method: 提出CCELLA方法，通过交叉注意力将非医学语言模型编码的文本特征与时间步嵌入的病理分类同时条件化LDM U-Net，并设计了联合损失函数和数据高效的训练框架。

Result: 在有限数据下，生成高质量医学图像，3D FID得分为0.025，显著优于基础模型（FID 0.071）。合成图像提升了前列腺癌分类器准确率（69%到74%），且仅用合成图像训练的分类器性能接近真实数据。

Conclusion: CCELLA方法有效解决了医学图像生成中的数据稀缺问题，提升了生成质量和分类器性能，具有较高的科学可访问性和实用性。
...
### [297] [Conditional diffusion models for guided anomaly detection in brain images using fluid-driven anomaly randomization](https://arxiv.org/abs/2506.10233)
...
<details>
  <summary>Details</summary>
Motivation: 监督学习需要大量疾病数据，但在罕见病等场景中难以获取。无监督异常检测方法（如扩散模型）虽能仅用健康图像训练，但常无法准确重建异常或健康区域。

Method: 采用弱监督方法，结合合成伪病理图像指导健康图像重建，通过流体驱动异常随机化生成逼真且解剖学一致的伪病理。

Result: 模型在合成和真实病理数据集（如ATLAS）上表现优异，优于变分自编码器、条件/无条件潜在扩散模型，甚至超过有监督修复方法。
...
### [298] [SWDL: Stratum-Wise Difference Learning with Deep Laplacian Pyramid for Semi-Supervised 3D Intracranial Hemorrhage Segmentation](https://arxiv.org/abs/2506.10325)
...
Method: 提出SWDL-Net框架，整合拉普拉斯金字塔的边缘锐化优势和深度卷积上采样的细节增强能力，通过差异学习机制实现互补优势的结合。
...
### [299] [ConStyX: Content Style Augmentation for Generalizable Medical Image Segmentation](https://arxiv.org/abs/2506.10675)
...
Method: ConStyX方法同时增强训练数据的内容和风格，并优化特征利用以减少过增强的负面影响。

Result: 实验表明，ConStyX在多域数据上表现出优越的泛化性能。

Conclusion: ConStyX通过内容和风格增强有效解决了域泛化问题，为医学图像分割提供了更鲁棒的解决方案。
...
### [300] [Generalist Models in Medical Image Segmentation: A Survey and Performance Comparison with Task-Specific Approaches](https://arxiv.org/abs/2506.10825)
...
<details>
  <summary>Details</summary>
Motivation: 随着通用模型在计算机视觉领域的成功，本文旨在全面调查其在医学图像分割中的发展、性能及潜在挑战。

Method: 通过分类分析SAM的零样本、少样本、微调等变体，并与其他创新模型及任务专用模型进行性能对比。
...
### [301] [Med-URWKV: Pure RWKV With ImageNet Pre-training For Medical Image Segmentation](https://arxiv.org/abs/2506.10858)
*Zhenhuan Zhou*
...
<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割方法（如CNN、Transformer及其混合架构）存在局限性，如CNN的受限感受野或Transformer的二次计算复杂度。RWKV模型因其线性计算复杂度和长程建模能力成为有前景的替代方案，但尚未充分探索预训练VRWKV模型的潜力。
...
Conclusion: Med-URWKV是医学领域首个纯RWKV分割模型，可直接重用预训练VRWKV编码器，性能优越，代码将开源。
...
### [302] [Semi-Automated Quality Assurance in Digital Pathology: Tile Classification Approach](https://arxiv.org/abs/2506.10916)
...
Method: 采用InceptionResNet模型，通过不同切片尺寸和放大倍数的消融研究，训练单伪影二元模型和多实例模型，提出混合设计优化伪影检测。

Result: 研究基于133张全幻灯片图像，提出混合模型设计，结合单伪影和多实例模型，显著提高了伪影检测的效率和准确性。

Conclusion: 混合设计结合单伪影和多实例模型，为数字病理学中的质量保证提供了一种高效且可扩展的解决方案。
